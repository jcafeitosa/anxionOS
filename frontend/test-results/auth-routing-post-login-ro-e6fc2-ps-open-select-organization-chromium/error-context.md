...
- Name: auth-routing.spec.ts >> post-login routing (live Better Auth) >> multiple memberships open select-organization
- Location: e2e/auth-routing.spec.ts:35:2
...
Expected pattern: /\/select-organization$/
Received string:  "http://127.0.0.1:4321/login?email=multi%40anxionos.local&password=anxionos-dev-pass"
Timeout: 20000ms

Call log:
  - Expect "toHaveURL" with timeout 20000ms
    43 × locator resolved to <html lang="pt-BR">…</html>
       - unexpected value "http://127.0.0.1:4321/login?email=multi%40anxionos.local&password=anxionos-dev-pass"
