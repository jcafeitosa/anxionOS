...
| **Gate atual** | G1 (slice 1-3 ✅ · slice 4 ✅ PASS · slice 5 next) |
| **Prioridade** | medium |
...
| Bootstrap API | ✅ Slice 4 | `ensureEventingSchema` → `ensureIdentitySchema` → `ensureOrganizationsSchema`; fail-fast pepper |
| Testes unitários | ✅ 30 pass | `bun test tests/organizations` (28 unit + 2 PG com flag) |
| Integração UoW P-R5-06 | ✅ | `integration/uow-journal-outbox.test.ts` 2/2 com RUN_PG_INTEGRATION_TESTS=true |
| Matriz G3/G5 | ❌ gap | G3-01..10, G5-01..04 não executados |
...
| **5** | Identity/governance wiring | `IdentityPrincipalLookup` real; consumer governance membership parity | integração identity lookup; eventos consumidos |
| **6** | G3/G5 oracles | Executar G3-01..10, G5-01..04 com fixture `orgs-two-agencies.json` | relatório evidência na issue |
