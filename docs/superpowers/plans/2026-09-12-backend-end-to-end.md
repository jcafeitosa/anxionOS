...
- [x] `ANX-513`: knowledge memory lifecycle now uses a tenant-scoped PostgreSQL `knowledge_memories` store with natural-key deduplication, candidate→promoted persistence, transaction-bound promotion, and fixture isolation; regular suite passes 1852/1852 and fresh PostgreSQL oracle passes 1890/1890.
- [x] `ANX-514`: agents no longer export an in-memory orchestration run fence; the fixture is isolated under tests and the domain port documents composition-root injection of orchestration-owned authority; agents/openbot verification passes 108/108; regular suite passes 1853/1853 with 38 integration skips, and fresh PostgreSQL oracle passes 1891/1891.
- [x] `ANX-515`: agents expose `GET /v1/agencies/{agencyId}/agents` with session and active-membership guards, tenant/agency filtering, deterministic ordering, OpenAPI catalog parity, handler/route coverage, and PostgreSQL HTTP proof; regular suite passes 1858/1858 with 38 integration skips, and fresh PostgreSQL oracle passes 1896/1896.
- [x] `ANX-516`: all 16 agents command-journal writers persist a canonical SHA-256 intent fingerprint and validate command name, target aggregate, and payload before replay; legacy rows without a fingerprint fail closed. Targeted agents tests pass 48/48 and the fresh PostgreSQL oracle passes 1900/1900 with 0 failures and 0 skips.
- [x] `ANX-517`: connections now hash command/inference intent, scope command journal lookups by organization, fail closed on legacy or divergent replay, and serialize idempotency keys transactionally; PostgreSQL concurrency proves one simulated provider call, one usage record, one inference request, and one account journal. Regular suite passes 1865/1865 with 38 integration skips; fresh PostgreSQL oracle passes 1903/1903 with 0 failures and 0 skips.
- [x] `ANX-518`: market-data command writers now persist canonical intent fingerprints, use organization-scoped journal replay with transactional advisory locks, and reject divergent or legacy replay with `MD_IDEMPOTENCY_CONFLICT`; helper tests cover canonical hashing and tenant isolation, while market-data integration coverage exercises real Timescale/PostgreSQL concurrency. Regular suite passes 1869/1869 with 38 integration skips; fresh PostgreSQL oracle passes 1907/1907 with 0 failures and 0 skips.
- [x] `ANX-519`: billing command writers and the usage consumer now persist canonical semantic intent fingerprints, scope journal reads by organization, serialize command/webhook/usage keys transactionally, and reject divergent or legacy replay with `BIL_IDEMPOTENCY_CONFLICT`; unit and real PostgreSQL persistence/conflict tests pass. Billing slice passes 16/16 targeted tests plus 2/2 PostgreSQL checks; regular suite passes 1874/1874 with 38 integration skips, and fresh PostgreSQL oracle passes 1912/1912 with 0 failures and 0 skips.
- [x] `ANX-520`: partners command writers now persist canonical semantic intent fingerprints, scope journal reads by organization, serialize command and invoice keys transactionally, and reject divergent or legacy replay with `PTR_IDEMPOTENCY_CONFLICT`; same-command cross-organization and concurrent replay behavior are proven against real PostgreSQL. Partners slice passes 18/18 targeted tests plus 2/2 PostgreSQL checks; fresh PostgreSQL oracle passes 1918/1918 with 0 failures and 0 skips. The separate `invoice.paid` versus `invoice.issued` projector semantic gap remains tracked.
- [x] `ANX-521`: billing now publishes `billing.invoice.paid.v1` only after a valid payment webhook transitions an issued invoice to PAID; partners consumes paid facts for accrual, no longer exposes the invoice.issued accrual consumer, and consumes `billing.refund.processed.v1` with refundId for one-time reversal. Module tests pass 35/35; real PostgreSQL end-to-end paid/refund flow passes 1/1; fresh PostgreSQL oracle passes 1921/1921 with 0 failures and 0 skips.
- [x] `ANX-522`: partners payout state is now canonical `SCHEDULED → PROCESSING → SETTLED | FAILED | REVERSED`; PostgreSQL migration 0003 records attempt/timestamps/provider references, commands use row locks plus organization-scoped intent journals, and FAILED retry emits one processing event per effective attempt. Partners module tests pass 21/21; real PostgreSQL paid/refund+payout integration passes 2/2; fresh PostgreSQL oracle passes 1925/1925 with 0 failures and 0 skips.
