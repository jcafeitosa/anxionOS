...
## Decisão pós-R10 — revogação de grant limitada ao alvo, emissor registrado e fim de `organization` (ANX-469)

Registrada em 2026-09-11 a partir do achado **MEDIUM** do executor do ANX-466 (item fora de escopo, de propósito) e das duas LOWs da mesma revalidação. Fecha o caminho de **remoção** de grants: onde a emissão foi endurecida com matriz de papel + catálogo + posse, a revogação tinha ficado apenas com a guarda genérica de papel.

**Problema 1 — alvo não limitado.** `DELETE /v1/agencies/:agencyId/grants/:grantId` reusava `requireAgencyMutationRole` (`owner|admin|operator`, via `assertActorCanMutate`) e não olhava o grant: um `operator` da agência revogava grants do `owner` da própria agência. O `capability-manifest` declara `governance.grant.revoke` com `requiredGrants: ["governance.owner", "governance.issuer"]` — a implementação não verificava nem o papel de owner nem a posse de emissor.

**Problema 2 — emissor não registrado.** A regra "ou issuer" era **inimplementável**: `governance_grants` não tinha coluna de emissor. Verificado em 2026-09-11 que não havia dado existente para derivá-lo: `governance_command_journal` guarda apenas o *response snapshot* (sem ator), o evento `grant.issued.v1` não carrega emissor, e `derived_from_membership_id` só existe para a baseline CAP-B01 (que é derivada pelo sistema, não emitida por principal). Conclusão: exigia coluna + migration.

**Decisão.**

1. **Autorização no contrato, mesma fonte da emissão.** `roleMayRevokeGrant({ role, capability, actorIsIssuer })` vive em `@anxionos/contracts/governance/grant-capability-policy.ts` ao lado de `roleMayIssueGrantCapability`. `owner` (constante `GRANT_REVOCATION_OWNER_ROLES`) revoga qualquer grant da agência; qualquer outro papel de mutação (`admin`/`operator`) só revoga o grant que **ele mesmo emitiu**, e nunca um de classe superior à que pode emitir — a classe vem de `roleMayIssueGrantCapability`, **não** de uma segunda lista. O boundary chama o contrato e recusa com `GOV_INSUFFICIENT_AUTHORITY` (403) **antes de qualquer escrita**. A restrição a `owner` + emissor alinha a implementação ao `capability-manifest` e à decisão explícita registrada em ANX-473.
2. **Emissor de primeira classe, nullable por semântica.** `governance_grants.issued_by_principal_id UUID` (migration `0009`) e `Grant.issuedByPrincipalId: string | null`. O boundary de emissão grava o ator (`handleIssueGrant` → `input.actor.principalId`); `null` é o valor correto para grants **derivados pelo sistema** — baseline CAP-B01 do `membership.activated`, break-glass e filhos de `createDelegation` — que não têm principal emissor e por isso só são revogáveis por `owner`. O comando `issueGrant` recebe o campo como **obrigatório** (`string | null`, explícito em cada chamador) para que um chamador novo não herde um emissor implícito.
3. **`hasCapability` exige escopo explícito (LOW 1).** `scopeId` deixou de ser opcional e o ramo `undefined = qualquer escopo` foi removido; `matchesScope` virou igualdade direta. Não havia consumidor de produção omitindo escopo (os três passam `agencyId` ou `PLATFORM_SCOPE_ID`), e o teste que **afirmava** o footgun ("omitted scope keeps any-scope behaviour") foi substituído por um teste de escopo explícito: grant `console.platform` preso a agência não autoriza o escopo PLATAFORMA.
4. **`organization` removido de `governance_scope_kind` (LOW 2).** O valor não tinha produtor, dono nem consumidor de autorização (`hasCapability` filtra só `scope_id`) e criava a expectativa falsa de um escopo institucional inexistente. **Consulta ao banco `anxionos_g5r` antes da remoção: `SELECT count(*) FROM governance_grants WHERE scope_kind = 'organization'` = 0.** Migration `0010` recria o tipo como `('agency','platform')` de forma idempotente (só age se o rótulo existir) e **fail-closed** (se houver qualquer linha com o valor, o cast falha em vez de apagar dado). Contrato (`governanceScopeKindSchema`), enum Drizzle e fixtures atualizados; não existe segunda árvore com o valor.

**Consequência aceita.** Um grant antigo emitido por um `operator` com capability administrativa (estado possível antes do ANX-466) permanece **irrevogável pelo emissor** — só `owner` o remove. Grants legados sem emissor (todos os anteriores a esta migration) também passam a exigir `owner`; é o efeito pretendido de fechar o alvo, e não há como reconstruir o emissor de dado histórico. A rota DELETE passa a responder **403 `GOV_INSUFFICIENT_AUTHORITY`** para admin/operator sobre grant alheio; o corpo de erro é o mesmo envelope institucional.

**Impacto em clientes/contrato publicado.** O contrato `@anxionos/contracts/governance` exporta `roleMayRevokeGrant` e `GRANT_REVOCATION_OWNER_ROLES` (aditivos); `governanceScopeKindSchema` **perde** `organization` (breaking de contrato: nenhum produtor o emitia). O documento OpenAPI da rota `DELETE /v1/agencies/:agencyId/grants/:grantId` é do orquestrador (`apps/api/src/openapi-operations.ts`, arquivo controlado, não tocado): falta declarar a exigência de owner/issuer e o `403` provado por execução.
