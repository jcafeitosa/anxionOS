...
**Consequência aceita.** Um grant antigo emitido por um `operator` com capability administrativa (estado possível antes do ANX-466) permanece **irrevogável pelo emissor** — só `owner`/`admin` o removem. Grants legados sem emissor (todos os anteriores a esta migration) também passam a exigir `owner`/`admin`; é o efeito pretendido de fechar o alvo, e não há como reconstruir o emissor de dado histórico. A rota DELETE passa a responder **403 `GOV_INSUFFICIENT_AUTHORITY`** para operator sobre grant alheio; o corpo de erro é o mesmo envelope institucional.
