...
| Reativar a membership de um principal **suspenso** devolve 200 (o lookup de existência saiu de `/activate`) | LOW | **Aceito como decisão de produto (F-3 do G4), com justificativa corrigida.** A guarda de status é **factível hoje**: `PrincipalLookup.exists()` já usa `getPrincipalById`, que devolve `null` para principal não-ativo — *não* seria preciso um port novo (a versão anterior deste registro afirmava o contrário e estava errada). O risco real é "membership pré-armada", não acesso indevido: o suspenso autentica 401, a reativação na identidade é ato de plataforma, e o consentimento de agência (D-ORG-046) é do owner/admin, não da identidade. **Decisão:** manter o acoplamento fora deste módulo e reavaliar junto de ANX-481. |
