#!/usr/bin/env bun
/**
 * ANX-179 — Cryptofeed sandbox homologation oracle.
 *
 * Verifies: engines-sandbox profile slot cryptofeed-sandbox (:9058), build/up/health,
 * non-root, read-only root FS, internal network, structured logging, auth on API paths.
 *
 * Importers: package.json `anx179:cryptofeed-sandbox-homologation`
 */
...
function parseCli(argv) {
	const { values } = parseArgs({
		args: argv,
		options: {
			issue: { type: "string", default: "ANX-179" },
	...
	try {
		...
		return {
			ok:
				...
				body?.simulated === true &&
				body?.adapterId === "adapter-cryptofeed" &&
				body?.dataOnly === true,
			body,
			url,
			via: "docker-exec",
...
function probeApiAuth(engine, authToken) {
	...
	return {
		...
		acceptsWithAuth:
			...
			authedBody?.engine === "cryptofeed-sandbox" &&
			authedBody?.dataOnly === true,
	};
}
...
async function main() {
	...
	if (!staticReport.composeOk) {
		if (opts.json) {
			...
			console.error("ANX-179 homologation FAILED: compose static checks");
	...
	if (!opts["skip-build"]) {
		...
		if (!build.ok) {
			...
			else console.error("ANX-179 homologation FAILED: docker build");
			process.exit(1);
		}
	} else {
	...
	if (!up.ok) {
		if (opts.json) console.log(JSON.stringify(report, null, 2));
		else console.error("ANX-179 homologation FAILED: docker up");
		process.exit(1);
	}
	...
	if (opts.json) {
		...
		console.log(
			`ANX-179 Cryptofeed sandbox homologation: ${report.overallOk ? "PASS" : "FAIL"}`,
		);
		console.log(JSON.stringify(report, null, 2));
	}
