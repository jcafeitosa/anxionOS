 ...
 *
 * Verifies: engines-sandbox profile slot nautilus-sandbox (:9057), build/up/health,
...
function parseCli(argv) {
	const { values } = parseArgs({
		args: argv,
		options: {
			issue: { type: "string", default: "ANX-174" },
	...
	try {
		...
		return {
			ok:
				...
				body?.simulated === true &&
				body?.adapterId === "adapter-nautilus",
			body,
			url,
			via: "docker-exec",
		};
...
function probeApiAuth(engine, authToken) {
	...
	return {
		...
		acceptsWithAuth:
			...
			authedBody?.engine === "nautilus-sandbox",
	};
}
...
async function main() {
	...
	if (!staticReport.composeOk) {
		if (opts.json) {
			...
			console.error("ANX-174 homologation FAILED: compose static checks");
	...
	if (!opts["skip-build"]) {
		...
		if (!build.ok) {
			...
			else console.error("ANX-174 homologation FAILED: docker build");
			process.exit(1);
		}
	} else {
	...
	if (!up.ok) {
		if (opts.json) console.log(JSON.stringify(report, null, 2));
		else console.error("ANX-174 homologation FAILED: docker up");
		process.exit(1);
	}
	...
	if (opts.json) {
		...
		console.log(
			`ANX-174 Nautilus sandbox homologation: ${report.overallOk ? "PASS" : "FAIL"}`,
		);
		console.log(JSON.stringify(report, null, 2));
	}
