	...
	return {
		async findByCommandId(commandId) {
			const result = await client.query(
				`SELECT command_id, organization_id, command_name, client_order_id, response_snapshot
				 FROM execution_command_journal WHERE command_id = $1`,
				[commandId],
			);
		...
		async findByClientOrderId(organizationId, clientOrderId) {
			const result = await client.query(
				`SELECT command_id, organization_id, command_name, client_order_id, response_snapshot
				 ...
				 WHERE organization_id = $1 AND client_order_id = $2`,
		...
		async save(entry) {
			await client.query(
				`INSERT INTO execution_command_journal (
