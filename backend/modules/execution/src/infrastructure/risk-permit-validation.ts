...
async function tableExists(client: Pool | PoolClient, tableName: string): Promise<boolean> {
...
export function createPgRiskPermitValidationPort(client: Pool | PoolClient): RiskPermitValidationPort {
    return {
        async validatePermit(input) {
            ...
            const result = await client.query(`SELECT id, organization_id, intent_hash, authority_epoch, risk_epoch, status
				 FROM risk_permits WHERE id = $1`, [input.riskPermitId]);
            ...
            if (row.authority_epoch !== input.authorityEpoch ||
