import { type ExecutionCommandResult, type OpenExecutionSessionCommand } from "@anxionos/contracts/execution";
...
import { assertExecutionModuleModeSupported, executionCommandResultSchema, openExecutionSessionCommandSchema } from "@anxionos/contracts/execution";
...
async function ensureSimulatedVenueAdapter(ctx: ExecutionTransactionContext, organizationId: string) {
    const existing = await ctx.venueAdapterRefs.findSimulatedByOrganization(organizationId);
    if (existing)
        return existing.id;
    ...
    const ensured = await ctx.venueAdapterRefs.findSimulatedByOrganization(organizationId);
    if (!ensured) {
        throwExecutionError("EX_SESSION_NOT_FOUND", "simulated venue adapter seed failed");
    }
    return ensured.id;
}
...
export async function openExecutionSession(deps: OpenExecutionSessionDeps, input: OpenExecutionSessionCommand): Promise<ExecutionCommandResult> {
    ...
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        const venueAdapterRefId = await ensureSimulatedVenueAdapter(ctx, command.organizationId);
