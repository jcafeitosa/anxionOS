...
export interface VenueAdapterRefRepository {
    findSimulatedByOrganization(organizationId: string): Promise<VenueAdapterRefRecord | null>;
...
export interface ExecutionOrderRepository {
    findByClientOrderId(organizationId: string, clientOrderId: string): Promise<ExecutionOrderRecord | null>;
...
export interface ExecutionUnitOfWork {
    runInTransaction<T>(work: (ctx: ExecutionTransactionContext) => Promise<T>): Promise<T>;
}
