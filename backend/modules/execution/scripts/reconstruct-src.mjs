...
function fixImports(content) {
	let out = content.replace(/import type \{([^}]*)\} from "@anxionos\/contracts";/g, (_m, specifiers) => {
		...
		if (execution.length) parts.push(`import type { ${execution.join(", ")} } from "@anxionos/contracts/execution";`);
		return parts.join("\n");
	});
	out = out.replace(/import\s*\{([^}]*)\}\s*from\s*"@anxionos\/contracts";/g, (_match, specifiers) => {
		...
		if (execution.length) parts.push(`import { ${execution.join(", ")} } from "@anxionos/contracts/execution";`);
		return parts.join("\n");
	});
