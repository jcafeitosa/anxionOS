...
function fixImports(content) {
	let out = content.replace(
		...
		(_m, specifiers) => {
			const { events, execution } = splitContractImports(specifiers);
			const parts = [];
			if (events.length)
				parts.push(
			...
			if (execution.length)
				parts.push(
					`import type { ${execution.join(", ")} } from "@anxionos/contracts/execution";`,
				);
			return parts.join("\n");
		},
	);
	out = out.replace(
		...
		(_match, specifiers) => {
			...
			if (execution.length)
				parts.push(
					`import { ${execution.join(", ")} } from "@anxionos/contracts/execution";`,
				);
			return parts.join("\n");
		},
	);
