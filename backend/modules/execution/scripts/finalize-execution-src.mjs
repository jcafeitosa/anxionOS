...
write("src/application/command-support.ts", `import type { ExecutionCommandResult } from "@anxionos/contracts/execution";
...
write("src/domain/events/execution-events.ts", `import { randomUUID } from "node:crypto";
...
write("src/infrastructure/execution-unit-of-work.ts", `import type { DomainEventEnvelope } from "@anxionos/contracts/events";
...
for (const [rel, fn, depsType, cmdType] of [
  ["src/application/commands/open-execution-session.ts", "openExecutionSession", "OpenExecutionSessionDeps", "OpenExecutionSessionCommand"],
  ["src/application/commands/submit-order.ts", "submitOrder", "SubmitOrderDeps", "SubmitOrderCommand"],
]) {
  let c = read(rel);
...
function mapVenueAdapterRef`,
    ...
    .replace("function mapVenueAdapterRef(row) {", "function mapVenueAdapterRef(row: Record<string, unknown>): VenueAdapterRefRecord {")
    .replace("function mapSession(row) {", "function mapSession(row: Record<string, unknown>): ExecutionSessionRecord {")
    .replace("function mapOrder(row) {", "function mapOrder(row: Record<string, unknown>): ExecutionOrderRecord {")
    .replace("function mapFill(row) {", "function mapFill(row: Record<string, unknown>): ExecutionFillRecord {")
    ...
    .replace(/adapterKind: row\.adapter_kind,/g, "adapterKind: String(row.adapter_kind),")
    ...
    .replace(/riskPermitId: row\.risk_permit_id,/g, "riskPermitId: String(row.risk_permit_id),")
    ...
    .replace(/venueAdapterRefId: row\.venue_adapter_ref_id,/g, "venueAdapterRefId: String(row.venue_adapter_ref_id),")
    .replace(/sessionId: row\.session_id,/g, "sessionId: String(row.session_id),")
    .replace(/clientOrderId: row\.client_order_id,/g, "clientOrderId: String(row.client_order_id),")
    ...
    .replace(/venueFillId: row\.venue_fill_id,/g, "venueFillId: String(row.venue_fill_id),")
    .replace(/asset: row\.asset,/g, "asset: String(row.asset),")
    .replace(/filledAt: new Date\(row\.filled_at\)\.toISOString\(\)/g, "filledAt: new Date(String(row.filled_at)).toISOString()")
    .replace(
    ...
    .replace(/async save\(record\)/g, "async save(record: ExecutionFillRecord)"),
);
