...
interface LruListNode<T> {
	key: string;
	value: GraphCacheEntry<T>;
	prev: LruListNode<T> | null;
	next: LruListNode<T> | null;
}
class LruTtlCache {
...
export function resolveGraphCacheMode(
...
export function createGraphReadCache(
	...
	const l1 = new LruTtlCache(
		options.l1MaxEntries ?? 500,
		options.l1TtlSeconds ?? GRAPH_CACHE_TTL_SECONDS.L1,
	);
	const l2 =
		mode === "l1-l2" && options.redis
...
export function createGraphCacheRebuildControl(
...
export function createPostAckCacheInvalidationHook(
	cache: GraphReadCache,
): (input: PostAckCacheInvalidationInput) => Promise<void> {
	return async (input) => {
		const message = {
			v: 1 as const,
...
export async function startGraphCacheInvalidationListener(
	cache,
	redis,
	options,
) {
