import { type CacheableTraversalId, type GraphCacheKeyParts } from "@anxionos/contracts/graph";
import { createHash } from "node:crypto";
import { buildGraphCacheRedisKey, graphCacheKeyPartsSchema, } from "@anxionos/contracts/graph";
...
export function buildScopeHash(scopeType: string, scopeId: string, principalId: string): string {
...
export function buildQueryHash(queryParams: Record<string, unknown>): string {
    return createHash("sha256").update(stableSerialize(queryParams)).digest("hex");
}
export function buildGraphCacheKeyParts(input: BuildGraphCacheKeyInput): GraphCacheKeyParts {
    return graphCacheKeyPartsSchema.parse({
        traversalId: input.traversalId,
        scopeHash: buildScopeHash(input.scopeType, input.scopeId, input.principalId),
...
export function buildGraphCacheRedisKeyFromInput(input: BuildGraphCacheKeyInput): string {
