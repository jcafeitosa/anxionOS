...
import type { GraphNeighborEdge, GraphStore } from "../../../domain/ports/graph-store";
...
function nodePropertiesToRecord(properties) {
    return {
...
function recordToGraphNode(record) {
...
function toWriteProperties(record, eventId) {
...
export function createNeo4jGraphStore(driver: Driver): GraphStore {
    return {
        async getNode(nodeKey) {
            ...
            try {
                const result = await session.run(`MATCH (n:GraphNode { nodeKey: $nodeKey })
					 RETURN n`, { nodeKey: formatNodeKey(nodeKey) });
        ...
        async getNodes(nodeKeys) {
            ...
            const session = driver.session();
            try {
                const result = await session.run(`UNWIND $nodeKeys AS key
					 MATCH (n:GraphNode { nodeKey: key })
					 RETURN n`, { nodeKeys: nodeKeys.map(formatNodeKey) });
        ...
        async upsertNode(record, eventId) {
            ...
            try {
                await session.run(`MERGE (n:GraphNode { nodeKey: $nodeKey })
					 ON CREATE SET
					   n = $properties,
        ...
        async deleteNode(nodeKey, eventId) {
            const session = driver.session();
            try {
                await session.run(`MATCH (n:GraphNode { nodeKey: $nodeKey })
        ...
        async listNeighbors(query) {
            ...
            try {
                if (query.direction !== "IN") {
                    const outResult = await session.run(`MATCH (n:GraphNode { nodeKey: $startKey })-[r]->(m:GraphNode)
                ...
                if (query.direction !== "OUT" && neighbors.length < query.maxResults) {
                    const inResult = await session.run(`MATCH (n:GraphNode { nodeKey: $startKey })<-[r]-(m:GraphNode)
						 WHERE $edgeTypes IS NULL OR type(r) IN $edgeTypes
						 RETURN type(r) AS edgeType, m.nodeKey AS targetNodeKey
						 LIMIT $limit`, {
                        startKey,
