...
} from "../../domain/ports/projection-inbox";
...
export async function findInboxEntry(queryable, eventId, consumerName) {
	const result = await queryable.query(
		`SELECT event_id, consumer_name, owner_domain, status, checkpoint,
...
export async function claimInboxForProcessing(client, input) {
	await client.query(
		`INSERT INTO graph_projection_inbox (
	...
	const claimed = await client.query(
		`UPDATE graph_projection_inbox
		  ...
		  RETURNING event_id, consumer_name, owner_domain, status, checkpoint,
		            projection_generation, attempt_count`,
		[input.eventId, input.consumerName, input.ownerDomain, input.checkpoint],
	);
...
export async function ackInboxEntry(
	client,
	eventId,
	consumerName,
	checkpoint,
	...
	await client.query(
		`UPDATE graph_projection_inbox
		    SET status = 'acked',
		        ...
		        error_code = NULL
		  WHERE event_id = $1 AND consumer_name = $2`,
		[eventId, consumerName, checkpoint, projectionGeneration],
	);
}
export async function markInboxPendingRetry(
	client,
	eventId,
	consumerName,
	errorCode,
	...
	await client.query(
		`UPDATE graph_projection_inbox
		    SET status = 'pending',
		        error_code = $3
		  WHERE event_id = $1 AND consumer_name = $2`,
		[eventId, consumerName, errorCode],
	);
}
export async function quarantineInboxEntry(
	client,
	eventId,
	consumerName,
	errorCode,
...
export async function bumpProjectionGeneration(client, consumerName) {
	const result = await client.query(
		`INSERT INTO graph_projection_generation (consumer_name, generation, updated_at)
		 VALUES ($1, 1, NOW())
