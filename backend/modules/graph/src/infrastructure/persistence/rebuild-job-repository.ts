...
import type { CreateRebuildJobInput, RebuildControl, RebuildJob, RebuildJobStatus } from "../../domain/ports/rebuild-control";
import { GRAPH_REBUILD_PENDING_BLOCK_THRESHOLD, GRAPH_REBUILD_TERMINAL_STATUSES, } from "../../domain/rebuild/constants";
...
export async function getCurrentGeneration(pool) {
    const result = await pool.query(`SELECT generation FROM graph_current_generation WHERE id = 'current'`);
    return Number(result.rows[0]?.generation ?? 0);
}
export async function getProjectionGeneration(pool, consumerName) {
    const result = await pool.query(`SELECT generation FROM graph_projection_generation WHERE consumer_name = $1`, [consumerName]);
    return Number(result.rows[0]?.generation ?? 0);
}
export async function getRegistryGeneration(pool) {
    const result = await pool.query(`SELECT generation FROM graph_registry_generation WHERE id = 'current'`);
...
export async function findActiveRebuildJob(pool) {
    const result = await pool.query(`SELECT job_id, status, target_generation, cutoff_checkpoint,
		  ...
		  LIMIT 1`, [GRAPH_REBUILD_TERMINAL_STATUSES]);
...
export async function createRebuildJobRecord(pool, input) {
    ...
    if (pending > GRAPH_REBUILD_PENDING_BLOCK_THRESHOLD) {
        throw new RebuildError(`Pending inbox ${pending} exceeds threshold ${GRAPH_REBUILD_PENDING_BLOCK_THRESHOLD}`, "REBUILD_PENDING_THRESHOLD");
    }
    const active = await findActiveRebuildJob(pool);
    if (active) {
        throw new RebuildError(`Rebuild job ${active.jobId} already active`, "REBUILD_ALREADY_ACTIVE");
    }
    const result = await pool.query(`INSERT INTO graph_rebuild_jobs (
...
export async function updateRebuildJobStatus(pool, jobId, status) {
    await pool.query(`UPDATE graph_rebuild_jobs
		    SET status = $2, updated_at = NOW()
		  WHERE job_id = $1`, [jobId, status]);
}
export async function swapCurrentGeneration(client, targetGeneration) {
    await client.query(`UPDATE graph_current_generation
		    SET generation = $1, updated_at = NOW()
		  WHERE id = 'current'`, [targetGeneration]);
}
...
export function createPgRebuildControl(pool: Pool): RebuildControl {
    return {
        ...
        getProjectionGeneration: (consumerName) => getProjectionGeneration(pool, consumerName),
        createRebuildJob: (input) => createRebuildJobRecord(pool, input),
        updateJobStatus: (jobId, status) => updateRebuildJobStatus(pool, jobId, status),
    };
}
