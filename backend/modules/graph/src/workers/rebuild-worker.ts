...
import type {
	ExecuteFullGenerationSwapInput,
	FullGenerationSwapResult,
	RebuildCacheControl,
	RebuildConsumerControl,
	RebuildF0Oracle,
	RebuildGenerationStore,
	RebuildReplayPort,
	StartFullGenerationSwapInput,
...
export async function runRebuildWorker(input) {
	...
	if (!leaderLock.acquired && !input.existingJob) {
		throw new RebuildError(
			`Rebuild leader lock held by job ${leaderLock.activeJobId}`,
	...
	if (input.existingJob) {
		...
		if (!fresh) {
			throw new RebuildError(
				`Rebuild job ${job.jobId} not found`,
				"REBUILD_NOT_FOUND",
			);
		}
	}
...
export interface RunRebuildWorkerInput
	extends Omit<ExecuteFullGenerationSwapInput, "job">,
		Omit<StartFullGenerationSwapInput, "pool" | "rebuildControl"> {
