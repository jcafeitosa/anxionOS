...
function parseAgencyCreated(payload) {
	if (
		...
		throw new ProjectionError(
			"Invalid agency.created payload",
			"SCHEMA_INVALID",
			"permanent",
		);
	}
...
function assertOrganizationsProjectionEnvelope(envelope: DomainEventEnvelope) {
	if (envelope.ownerDomain !== ORGANIZATIONS_OWNER_DOMAIN) {
		throw new ProjectionError(
			`Unexpected ownerDomain for organizations projection: ${envelope.ownerDomain}`,
	...
	try {
		...
		throw new ProjectionError(
			error instanceof Error ? error.message : "Invalid tenant scope",
			"SCHEMA_INVALID",
			"permanent",
		);
	}
...
export async function projectOrganizationsEvent(context) {
	...
	assertOrganizationsProjectionEnvelope(envelope);
	switch (envelope.eventType) {
		case ORGANIZATION_EVENT_TYPES.AGENCY_CREATED: {
			...
			await graphStore.upsertNode(
				toAgencyRecord(payload, projectionGeneration),
				envelope.eventId,
			);
			return;
		}
		case ORGANIZATION_EVENT_TYPES.MEMBERSHIP_ACTIVATED: {
			...
			await graphStore.upsertNode(
				toMembershipRecord(payload, projectionGeneration),
				envelope.eventId,
			);
			return;
		}
