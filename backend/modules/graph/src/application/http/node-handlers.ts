...
function parseNodeKeyParam(encodedKey) {
	try {
		...
		throw new GraphHttpError(
			...
			"Invalid nodeKey path parameter",
		);
	}
}
export async function handleNodeGet(runtime, nodeKeyParam, query, scope) {
	...
	if (!parsedQuery.success) {
		throw new GraphHttpError(
			"TRAVERSAL_INPUT_INVALID",
			"Invalid node.get query parameters",
	...
	if (!record) {
		if (pending && minGeneration !== undefined) {
			throw new GraphHttpError(
				"NODE_NOT_PROJECTED",
				"Node accepted but projection is pending",
				{
					nodeKey,
	...
	if (
		minGeneration !== undefined &&
		record.projectionGeneration < minGeneration
	) {
		throw new GraphHttpError(
			"NODE_NOT_PROJECTED",
			"Node projection generation below requested minimum",
			{
				nodeKey,
...
export async function handleNodesBatchGet(runtime, body, scope) {
	...
	if (!parsed.success) {
		throw new GraphHttpError(
			"TRAVERSAL_INPUT_INVALID",
			"Invalid nodes.batchGet payload",
	...
	if (keys.length > NODES_BATCH_GET_MAX_KEYS) {
		throw new GraphHttpError(
			"TRAVERSAL_INPUT_INVALID",
			`nodes.batchGet accepts at most ${NODES_BATCH_GET_MAX_KEYS} keys`,
			{ keyCount: keys.length, maxKeys: NODES_BATCH_GET_MAX_KEYS },
		);
	}
	...
	const recordByKey = new Map<
		string,
		import("../../domain/ports/graph-store").GraphNodeRecord
	>(
		records.map((record) => [
	...
	for (const nodeKey of keys) {
		...
		if (
			minProjectionGeneration !== undefined &&
