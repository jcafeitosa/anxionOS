...
export function assertPlatformScope(scopeType: string): void {
	if (scopeType !== "PLATFORM") {
		throw new GraphHttpError(
			"FORBIDDEN_SCOPE",
			"Admin graph operations require PLATFORM acting scope",
		);
	}
}
export async function handleAdminRebuild(runtime, body, actingScopeType) {
	...
	if (!parsed.success) {
		throw new GraphHttpError(
			...
			"auditManifestId is required",
			parsed.error.flatten(),
		);
	}
	try {
		const currentGeneration =
			await runtime.rebuildControl.getCurrentGeneration();
...
export async function handleAdminDlqReplay(
	runtime,
	dlqId,
	body,
	actingScopeType,
	...
	if (!entry) {
		throw new GraphHttpError("NODE_NOT_FOUND", "DLQ entry not found", {
			dlqId,
		});
	}
	const replayResult = await runtime.dlqReplay.replay(
		dlqId,
		parsed.data.auditManifestId,
	);
	if (replayResult === "replayed") {
		await runtime.dlqReplay.resetInboxForReplay(
			entry.eventId,
			entry.consumerName,
		);
	}
