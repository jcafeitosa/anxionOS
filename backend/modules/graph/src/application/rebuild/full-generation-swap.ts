...
export async function startFullGenerationSwap(input) {
    ...
    const registryGeneration = input.registryGeneration ?? (await input.rebuildRegistry.getRegistryGeneration());
...
async function finalizeSwap(rebuildRegistry, rebuildControl, job, generationStore) {
...
export async function executeFullGenerationSwap(input) {
    ...
    try {
        ...
        await finalizeSwap(input.rebuildRegistry, rebuildControl, job, input.generationStore);
        await input.onPhase?.("running");
        await input.consumerControl.resume();
        input.cacheControl?.setCacheWriteEnabled(true);
        const currentGeneration = await input.rebuildRegistry.getCurrentGeneration();
        if (currentGeneration !== job.targetGeneration) {
            throw new RebuildError(`Generation swap mismatch: expected ${job.targetGeneration}, got ${currentGeneration}`, "REBUILD_SWAP_MISMATCH");
        }
        return {
...
export type FullGenerationSwapPhase = "draining" | "paused" | "rebuilding" | "flushing" | "verifying" | "swapping" | "running";
