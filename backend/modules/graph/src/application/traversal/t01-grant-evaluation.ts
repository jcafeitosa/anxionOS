import type { NodeKey, ScopeContext, T01Input, T01Output, T03Output } from "@anxionos/contracts/graph";
...
function parsePayloadField(payload, field) {
...
export function resolveGrantScope(scope: ScopeContext, resourceNodeKey: NodeKey): GrantScope {
...
export function grantMatchesT01(grant: GraphNodeRecord, actorId: string, action: string, expectedAuthorityEpoch?: number): boolean {
    if (grant.nodeKey.type !== "Grant" || grant.status !== "active") {
        return false;
    }
    const granteePrincipalId = parsePayloadField(grant.payload, "granteePrincipalId");
...
export function filterMatchingGrants(grants: GraphNodeRecord[], actorId: string, action: string, expectedAuthorityEpoch?: number): GraphNodeRecord[] {
    return grants.filter((grant) => grantMatchesT01(grant, actorId, action, expectedAuthorityEpoch));
}
export function evaluateT01Grants(input: T01GrantEvaluationInput, grants: GraphNodeRecord[]): T01GrantEvaluationResult {
    ...
    const scopedGrants = grants.filter((grant) => grant.nodeKey.scopeType === grantScope.scopeType &&
        grant.nodeKey.scopeId === grantScope.scopeId);
    const matched = filterMatchingGrants(scopedGrants, params.actorId, params.action, params.expectedAuthorityEpoch);
    if (matched.length === 0) {
        return {
            output: {
                decision: "DENY",
    ...
    const authorityEpoch = params.expectedAuthorityEpoch ??
        parsePayloadField(matched[0].payload, "authorityEpoch");
    const projectionGeneration = matched.reduce((max, grant) => Math.max(max, grant.projectionGeneration), 0);
    return {
        output: {
            decision: "ALLOW",
            authorityEpoch: typeof authorityEpoch === "number" ? authorityEpoch : Number(authorityEpoch),
            riskEpoch: params.expectedRiskEpoch,
            proof: { grantIds },
        },
...
export function toT03Output(t01: T01Output): T03Output {
    ...
    const message = t01.decision === "ALLOW"
