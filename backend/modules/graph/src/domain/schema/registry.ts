...
export function createGraphSchemaRegistry(input: GraphSchemaRegistryInput): GraphSchemaRegistry {
    ...
    const requireNodeType = (nodeType, schemaVersion) => {
        ...
        if (!def) {
            throw new GraphSchemaRegistryError("UNKNOWN_NODE_TYPE", `Node type not registered: ${nodeType}@${schemaVersion}`);
        }
        return def;
    };
    const requireEdgeType = (edgeTypeId) => {
        ...
        if (!def) {
            throw new GraphSchemaRegistryError("UNKNOWN_EDGE_TYPE", `Edge type id not registered: ${edgeTypeId}`);
        }
        return def;
    };
    const requireEdgeTypeByName = (edgeType) => {
        ...
        if (!def) {
            throw new GraphSchemaRegistryError("EDGE_NOT_REGISTERED", `Edge type not registered in allowlist: ${edgeType}`);
        }
        return def;
    };
    return {
...
export function createInMemoryGraphSchemaRegistry(nodeTypes: NodeTypeDef[], edgeTypes: EdgeTypeDef[]): GraphSchemaRegistry {
