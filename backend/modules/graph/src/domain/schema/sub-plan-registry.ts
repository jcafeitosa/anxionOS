...
export function createSubPlanRegistry(
	...
	const register = (plan) => {
		...
		if (plans.has(key)) {
			throw new GraphSchemaRegistryError(
				"DUPLICATE_SUBPLAN",
				`Sub-plan already registered: ${plan.traversalId}/${plan.planId}`,
			);
		}
		try {
			...
			if (error instanceof GraphSchemaRegistryError) {
				throw new GraphSchemaRegistryError(
					...
					`Sub-plan ${plan.planId} references unregistered edge in allowlist: ${error.message}`,
					{ cause: error },
				);
			}
			throw error;
	...
	const require = (traversalId, planId) => {
		...
		if (!plan) {
			throw new GraphSchemaRegistryError(
				...
				`Sub-plan not registered: ${traversalId}/${planId}`,
			);
		}
		return plan;
	};
