...
function fixImports(content) {
	let out = content.replace(/import type \{([^}]*)\} from "@anxionos\/contracts";/g, (_m, specifiers) => {
		...
		if (billing.length) parts.push(`import type { ${billing.join(", ")} } from "@anxionos/contracts/billing";`);
		return parts.join("\n");
	});
	out = out.replace(/import\s*\{([^}]*)\}\s*from\s*"@anxionos\/contracts";/g, (_match, specifiers) => {
		...
		if (billing.length) parts.push(`import { ${billing.join(", ")} } from "@anxionos/contracts/billing";`);
		return parts.join("\n");
	});
