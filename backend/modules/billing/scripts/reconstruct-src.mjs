...
function fixImports(content) {
	let out = content.replace(
		...
		(_m, specifiers) => {
			const { events, billing } = splitContractImports(specifiers);
			const parts = [];
			if (events.length)
				parts.push(
			...
			if (billing.length)
				parts.push(
					`import type { ${billing.join(", ")} } from "@anxionos/contracts/billing";`,
				);
			return parts.join("\n");
		},
	);
	out = out.replace(
		...
		(_match, specifiers) => {
			...
			if (billing.length)
				parts.push(
					`import { ${billing.join(", ")} } from "@anxionos/contracts/billing";`,
				);
			return parts.join("\n");
		},
	);
