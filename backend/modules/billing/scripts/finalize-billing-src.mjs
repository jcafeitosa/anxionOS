...
write("src/application/command-support.ts", `import type { BillingCommandResult } from "@anxionos/contracts/billing";
...
write("src/domain/events/billing-events.ts", `import { randomUUID } from "node:crypto";
...
write("src/infrastructure/billing-unit-of-work.ts", `import type { DomainEventEnvelope } from "@anxionos/contracts/events";
...
for (const [rel, fn, depsType, cmdType] of [
  ["src/application/commands/create-subscription.ts", "createSubscription", "CreateSubscriptionDeps", "CreateSubscriptionCommand"],
  ["src/application/commands/issue-invoice.ts", "issueInvoice", "IssueInvoiceDeps", "IssueInvoiceCommand"],
]) {
  let c = read(rel);
...
if (!consumer.includes("createUsageRecordedConsumer(deps: UsageRecordedConsumerDeps)")) {
  consumer = consumer.replace(
...
function mapSubscription`,
    ...
    .replace("function mapSubscription(row) {", "function mapSubscription(row: Record<string, unknown>): SubscriptionRecord {")
    ...
    .replace(/billingPeriodStart: row\.billing_period_start\.toISOString\(\)/g, "billingPeriodStart: (row.billing_period_start as Date).toISOString()")
    .replace(/billingPeriodEnd: row\.billing_period_end\.toISOString\(\)/g, "billingPeriodEnd: (row.billing_period_end as Date).toISOString()")
    .replace(/status: row\.status,/g, "status: String(row.status),")
    .replace("function mapInvoice(row) {", "function mapInvoice(row: Record<string, unknown>): InvoiceRecord {")
    .replace(/subscriptionId: row\.subscription_id,/g, "subscriptionId: String(row.subscription_id),")
    .replace(/billingPeriod: row\.billing_period,/g, "billingPeriod: String(row.billing_period),")
    .replace(/issuedAt: row\.issued_at \? row\.issued_at\.toISOString\(\) : null/g, "issuedAt: row.issued_at ? (row.issued_at as Date).toISOString() : null")
    .replace("function mapInvoiceLine(row) {", "function mapInvoiceLine(row: Record<string, unknown>): InvoiceLineRecord {")
    .replace(/invoiceId: row\.invoice_id,/g, "invoiceId: String(row.invoice_id),")
    .replace(/usageRecordId: row\.usage_record_id,/g, "usageRecordId: String(row.usage_record_id),")
    .replace(/description: row\.description,/g, "description: String(row.description),")
    .replace("function mapUsageAggregation(row) {", "function mapUsageAggregation(row: Record<string, unknown>): UsageAggregationRecord {")
    .replace(/unit: row\.unit,/g, "unit: String(row.unit),")
    .replace(/consumerKind: row\.consumer_kind,/g, "consumerKind: String(row.consumer_kind),")
    .replace(
    ...
    .replace(/async save\(record\)/g, "async save(record: SubscriptionRecord | InvoiceRecord | InvoiceLineRecord | UsageAggregationRecord)"),
);
