...
export interface SubscriptionRepository {
    ...
    findActiveByOrganizationAndPlan(organizationId: string, planCode: string): Promise<SubscriptionRecord | null>;
    save(record: SubscriptionRecord): Promise<SubscriptionRecord>;
}
export interface InvoiceRepository {
    findById(id: string): Promise<InvoiceRecord | null>;
    findDraftBySubscriptionAndPeriod(subscriptionId: string, billingPeriod: string): Promise<InvoiceRecord | null>;
    save(record: InvoiceRecord): Promise<InvoiceRecord>;
    updateStatus(id: string, status: string, issuedAt: string | null): Promise<InvoiceRecord>;
...
export interface UsageAggregationRepository {
    findByUsageRecordId(usageRecordId: string): Promise<UsageAggregationRecord | null>;
...
export interface BillingUnitOfWork {
    runInTransaction<T>(work: (ctx: BillingTransactionContext) => Promise<T>): Promise<T>;
}
