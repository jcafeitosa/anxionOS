...
export function createPgSubscriptionRepository(client: PoolClient): SubscriptionRepository {
    return {
        async findById(id) {
            const result = await client.query(`SELECT * FROM billing_subscriptions WHERE id = $1`, [id]);
            const row = result.rows[0];
            return row ? mapSubscription(row) : null;
        },
        async findActiveByOrganizationAndPlan(organizationId, planCode) {
            const result = await client.query(`SELECT * FROM billing_subscriptions
			 WHERE organization_id = $1 AND plan_code = $2 AND status = 'ACTIVE'
			 ORDER BY created_at DESC LIMIT 1`, [organizationId, planCode]);
            const row = result.rows[0];
            return row ? mapSubscription(row) : null;
        },
...
export function createPgInvoiceRepository(client: PoolClient): InvoiceRepository {
    return {
        ...
        async findDraftBySubscriptionAndPeriod(subscriptionId, billingPeriod) {
            const result = await client.query(`SELECT * FROM billing_invoices
			 WHERE subscription_id = $1 AND billing_period = $2 AND status = 'DRAFT'
			 ORDER BY created_at DESC LIMIT 1`, [subscriptionId, billingPeriod]);
            const row = result.rows[0];
            return row ? mapInvoice(row) : null;
        },
        ...
        async updateStatus(id, status, issuedAt) {
            const result = await client.query(`UPDATE billing_invoices SET status = $2, issued_at = $3
			 WHERE id = $1 RETURNING *`, [id, status, issuedAt]);
            return mapInvoice(result.rows[0]);
        },
        async updateTotalAmount(id, totalAmount) {
            await client.query(`UPDATE billing_invoices SET total_amount = $2 WHERE id = $1`, [
                id,
                totalAmount,
            ]);
        },
...
export function createPgInvoiceLineRepository(client: PoolClient): InvoiceLineRepository {
    return {
        async findByUsageRecordId(usageRecordId) {
            const result = await client.query(`SELECT * FROM billing_invoice_lines WHERE usage_record_id = $1`, [usageRecordId]);
            const row = result.rows[0];
            return row ? mapInvoiceLine(row) : null;
        },
        async save(record: InvoiceLineRecord) {
            await client.query(`INSERT INTO billing_invoice_lines (
        ...
        async sumAmountByInvoice(invoiceId) {
            const result = await client.query(`SELECT COALESCE(SUM(amount), 0) AS total FROM billing_invoice_lines WHERE invoice_id = $1`, [invoiceId]);
...
export function createPgUsageAggregationRepository(client: PoolClient): UsageAggregationRepository {
    return {
        async findByUsageRecordId(usageRecordId) {
            const result = await client.query(`SELECT * FROM billing_usage_aggregations WHERE usage_record_id = $1`, [usageRecordId]);
            const row = result.rows[0];
            return row ? mapUsageAggregation(row) : null;
        },
        async save(record: UsageAggregationRecord) {
            await client.query(`INSERT INTO billing_usage_aggregations (
