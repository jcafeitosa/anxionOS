	...
	return {
		async findByCommandId(commandId) {
			const result = await client.query(
				`SELECT command_id, organization_id, command_name, usage_record_id, response_snapshot
		...
		async save(entry) {
			await client.query(
				`INSERT INTO billing_command_journal (command_id, organization_id, command_name, usage_record_id, response_snapshot)
			 VALUES ($1,$2,$3,$4,$5)`,
				[
					entry.commandId,
