...
export function createUsageRecordedConsumer(deps: UsageRecordedConsumerDeps): {
  ...
  return {
    async handle(usage: ConnectionsUsageRecordedBridge) {
      ...
      const existingByUsage = await deps.commandJournal.findByUsageRecordId(command.usageRecordId);
      if (existingByUsage && existingByUsage.organizationId !== command.organizationId) {
        throwBillingError("BIL_CROSS_TENANT", "usage record organization mismatch");
      }
      const replayByUsage = await loadIdempotentByUsageRecordId(deps.commandJournal, command.usageRecordId);
      if (replayByUsage) return replayByUsage;
      return deps.unitOfWork.runInTransaction(async (ctx) => {
        const racedByUsage = await ctx.commandJournal.findByUsageRecordId(command.usageRecordId);
        if (racedByUsage) {
          if (racedByUsage.organizationId !== command.organizationId) {
            throwBillingError("BIL_CROSS_TENANT", "usage record organization mismatch");
          }
          const parsed = parseCommandResultSnapshot(racedByUsage.responseSnapshot);
          return billingCommandResultSchema.parse({ ...parsed, idempotentReplay: true });
        }
        const existingAggregation = await ctx.usageAggregations.findByUsageRecordId(command.usageRecordId);
        if (existingAggregation) {
          if (existingAggregation.organizationId !== command.organizationId) {
            throwBillingError("BIL_CROSS_TENANT", "usage aggregation organization mismatch");
          }
          const line = await ctx.invoiceLines.findByUsageRecordId(command.usageRecordId);
        ...
        const subscription = await ctx.subscriptions.findById(command.subscriptionId);
        if (!subscription || subscription.organizationId !== command.organizationId) {
          throwBillingError("BIL_SUBSCRIPTION_NOT_FOUND", "subscription not found for organization");
        }
        let invoice = await ctx.invoices.findDraftBySubscriptionAndPeriod(subscription.id, command.billingPeriod);
        if (!invoice) {
        ...
        const amount = multiplyDecimalAmount(command.quantity, command.unitPrice);
