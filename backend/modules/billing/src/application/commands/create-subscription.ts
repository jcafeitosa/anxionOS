import { type CreateSubscriptionCommand, type BillingCommandResult } from "@anxionos/contracts/billing";
import type { CommandJournalRepository } from "../../domain/ports/command-journal";
import type { BillingUnitOfWork } from "../../domain/ports/billing-unit-of-work";
import { randomUUID } from "node:crypto";
import { createSubscriptionCommandSchema, billingCommandResultSchema } from "@anxionos/contracts/billing";
...
export async function createSubscription(deps: CreateSubscriptionDeps, input: CreateSubscriptionCommand): Promise<BillingCommandResult> {
    ...
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        const existing = await ctx.subscriptions.findActiveByOrganizationAndPlan(command.organizationId, command.planCode);
        if (existing) {
