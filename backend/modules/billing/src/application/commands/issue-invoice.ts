import { type IssueInvoiceCommand, type BillingCommandResult } from "@anxionos/contracts/billing";
import type { CommandJournalRepository } from "../../domain/ports/command-journal";
import type { BillingUnitOfWork } from "../../domain/ports/billing-unit-of-work";
import { issueInvoiceCommandSchema, billingCommandResultSchema } from "@anxionos/contracts/billing";
...
export async function issueInvoice(deps: IssueInvoiceDeps, input: IssueInvoiceCommand): Promise<BillingCommandResult> {
    ...
    if (existingCommand && existingCommand.organizationId !== command.organizationId) {
        throwBillingError("BIL_CROSS_TENANT", "command journal organization mismatch");
    }
    ...
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        if (invoice.subscriptionId !== command.subscriptionId) {
            throwBillingError("BIL_INVOICE_NOT_FOUND", "invoice does not belong to subscription");
        }
        ...
        if (invoice.status !== "DRAFT") {
            throwBillingError("BIL_INVOICE_NOT_FOUND", "invoice is not in DRAFT status");
        }
        const totalAmount = await ctx.invoiceLines.sumAmountByInvoice(invoice.id);
        const issuedAt = new Date().toISOString();
        const issued = await ctx.invoices.updateStatus(invoice.id, "ISSUED", issuedAt);
