...
export function throwSimulationError(code: SimulationErrorCode, message: string): never {
  throw new SimulationCommandError(code, message);
}

export function parseCommandResultSnapshot(snapshot: Record<string, unknown>): SimulationCommandResult {
