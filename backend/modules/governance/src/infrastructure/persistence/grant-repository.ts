...
import { grants, type GrantRow } from "./schema";
...
export function createDrizzleGrantRepository(
	...
	return {
		async save(grant: Grant) {
			const existing = await db.select().from(grants).where(eq(grants.id, grant.id)).limit(1);
			if (existing[0]) {
				...
				const rows = await db
					...
					.where(and(eq(grants.id, grant.id), eq(grants.revision, expectedRevision)))
					.returning();
				const row = rows[0];
				if (!row) {
		...
		async findActiveByDerivedFromMembershipId(membershipId: string) {
			const rows = await db
				...
				.from(grants)
				.where(
					and(eq(grants.derivedFromMembershipId, membershipId), eq(grants.status, "active")),
				);
			return rows.map(toGrant);
		},
	};
