import { type OperationsCommandResult, type RegisterHealthCheckCommand } from "@anxionos/contracts/operations";
import type { CommandJournalRepository } from "../../domain/ports/command-journal";
import type { OperationsUnitOfWork } from "../../domain/ports/operations-unit-of-work";
import { randomUUID } from "node:crypto";
import { operationsCommandResultSchema, registerHealthCheckCommandSchema } from "@anxionos/contracts/operations";
import { createHealthDegradedEvent, isDegradedHealthStatus, } from "../../domain/events/operations-events";
...
export async function registerHealthCheck(deps: RegisterHealthCheckDeps, input: RegisterHealthCheckCommand): Promise<OperationsCommandResult> {
    ...
    if (existingCommand && existingCommand.organizationId !== command.organizationId) {
        throwOperationsError("OPS_CROSS_TENANT", "command journal organization mismatch");
    }
    ...
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        if (raced) {
            ...
            return operationsCommandResultSchema.parse({ ...parsed, idempotentReplay: true });
        }
        const existing = await ctx.healthChecks.findByOrganizationAndServiceId(command.organizationId, command.serviceId);
        if (existing) {
            if (existing.organizationId !== command.organizationId) {
                throwOperationsError("OPS_CROSS_TENANT", "health check organization mismatch");
            }
            ...
            if (isDegradedHealthStatus(command.status)) {
                events.push(createHealthDegradedEvent({
