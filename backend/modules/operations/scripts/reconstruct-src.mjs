...
function fixImports(content) {
	let out = content.replace(/import type \{([^}]*)\} from "@anxionos\/contracts";/g, (_m, specifiers) => {
		...
		if (operations.length) parts.push(`import type { ${operations.join(", ")} } from "@anxionos/contracts/operations";`);
		return parts.join("\n");
	});
	out = out.replace(/import\s*\{([^}]*)\}\s*from\s*"@anxionos\/contracts";/g, (_match, specifiers) => {
		...
		if (operations.length) parts.push(`import { ${operations.join(", ")} } from "@anxionos/contracts/operations";`);
		return parts.join("\n");
	});
