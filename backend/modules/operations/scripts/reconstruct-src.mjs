...
function fixImports(content) {
	let out = content.replace(
		...
		(_m, specifiers) => {
			const { events, operations } = splitContractImports(specifiers);
			const parts = [];
			if (events.length)
				parts.push(
			...
			if (operations.length)
				parts.push(
					`import type { ${operations.join(", ")} } from "@anxionos/contracts/operations";`,
				);
			return parts.join("\n");
		},
	);
	out = out.replace(
		...
		(_match, specifiers) => {
			...
			if (operations.length)
				parts.push(
					`import { ${operations.join(", ")} } from "@anxionos/contracts/operations";`,
				);
			return parts.join("\n");
		},
	);
