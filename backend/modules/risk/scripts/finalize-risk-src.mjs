...
write(
	"src/domain/events/risk-events.ts",
...
write(
	"src/infrastructure/risk-unit-of-work.ts",
	`import type { DomainEventEnvelope } from "@anxionos/contracts/events";
...
for (const [rel, fn, depsType, cmdType] of [
	[
		"src/application/commands/activate-limit-policy.ts",
		"activateLimitPolicy",
		"ActivateLimitPolicyDeps",
		"ActivateLimitPolicyCommand",
	],
	[
		"src/application/commands/run-pre-trade-check.ts",
		"runPreTradeCheck",
		"RunPreTradeCheckDeps",
...
function mapPolicy`,
		...
		.replace(
			/policyVersion: row\.policy_version,/g,
		...
		.replace(
			/currentRiskEpoch: row\.current_risk_epoch,/g,
		...
		.replace(
			/checkResult: row\.check_result,/g,
			"checkResult: String(row.check_result),",
		)
		.replace(
			/denyReasonCode: row\.deny_reason_code/g,
			"denyReasonCode: row.deny_reason_code != null ? String(row.deny_reason_code)",
		)
		...
		.replace(
			/async save\(record\)/g,
			"async save(record: LimitPolicyRecord | CheckResultRecord | PermitRecord)",
		),
);
