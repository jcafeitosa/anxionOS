	...
	return {
		async findByCommandId(commandId) {
			const result = await client.query(
				`SELECT command_id, organization_id, command_name, intent_hash, response_snapshot
				 FROM risk_command_journal WHERE command_id = $1`,
				[commandId],
			);
		...
		async save(entry) {
			await client.query(
				`INSERT INTO risk_command_journal (command_id, organization_id, command_name, intent_hash, response_snapshot)
				 VALUES ($1,$2,$3,$4,$5)`,
				[
					entry.commandId,
