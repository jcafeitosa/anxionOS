...
write("src/application/command-support.ts", `import type { PerformanceCommandResult } from "@anxionos/contracts/performance";
...
write("src/domain/events/performance-events.ts", `import { randomUUID } from "node:crypto";
...
write("src/infrastructure/performance-unit-of-work.ts", `import type { DomainEventEnvelope } from "@anxionos/contracts/events";
...
for (const [rel, fn, depsType, cmdType] of [
  ["src/application/commands/record-outcome-snapshot.ts", "recordOutcomeSnapshot", "RecordOutcomeSnapshotDeps", "RecordOutcomeSnapshotCommand"],
]) {
  let c = read(rel);
...
function mapOutcomeSnapshot`,
    ...
    .replace(/outcomeSnapshotId: row\.outcome_snapshot_id,/g, "outcomeSnapshotId: String(row.outcome_snapshot_id),")
