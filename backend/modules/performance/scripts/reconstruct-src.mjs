...
function fixImports(content) {
	let out = content.replace(/import type \{([^}]*)\} from "@anxionos\/contracts";/g, (_m, specifiers) => {
		...
		if (performance.length) parts.push(`import type { ${performance.join(", ")} } from "@anxionos/contracts/performance";`);
		return parts.join("\n");
	});
	out = out.replace(/import\s*\{([^}]*)\}\s*from\s*"@anxionos\/contracts";/g, (_match, specifiers) => {
		...
		if (performance.length) parts.push(`import { ${performance.join(", ")} } from "@anxionos/contracts/performance";`);
		return parts.join("\n");
	});
