...
function fixImports(content) {
	let out = content.replace(
		...
		(_m, specifiers) => {
			const { events, performance } = splitContractImports(specifiers);
			const parts = [];
			if (events.length)
				parts.push(
			...
			if (performance.length)
				parts.push(
					`import type { ${performance.join(", ")} } from "@anxionos/contracts/performance";`,
				);
			return parts.join("\n");
		},
	);
	out = out.replace(
		...
		(_match, specifiers) => {
			...
			if (performance.length)
				parts.push(
					`import { ${performance.join(", ")} } from "@anxionos/contracts/performance";`,
				);
			return parts.join("\n");
		},
	);
