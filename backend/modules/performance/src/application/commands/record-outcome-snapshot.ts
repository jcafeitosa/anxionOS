import { type PerformanceCommandResult, type RecordOutcomeSnapshotCommand } from "@anxionos/contracts/performance";
import type { CommandJournalRepository } from "../../domain/ports/command-journal";
import type { PerformanceUnitOfWork } from "../../domain/ports/performance-unit-of-work";
import { randomUUID } from "node:crypto";
import { performanceCommandResultSchema, recordOutcomeSnapshotCommandSchema } from "@anxionos/contracts/performance";
import { createOutcomeRecordedEvent } from "../../domain/events/performance-events";
import { loadIdempotentByJournalEntryId, loadIdempotentCommandResult, toCommandResultSnapshot, } from "../command-support";
...
export async function recordOutcomeSnapshot(deps: RecordOutcomeSnapshotDeps, input: RecordOutcomeSnapshotCommand): Promise<PerformanceCommandResult> {
    ...
    if (existingCommand && existingCommand.organizationId !== command.organizationId) {
        throwPerformanceError("PERF_CROSS_TENANT", "command journal organization mismatch");
    }
    const replayByCommand = await loadIdempotentCommandResult(deps.commandJournal, command.commandId);
    if (replayByCommand)
        return replayByCommand;
    const existingByEntry = await deps.commandJournal.findByJournalEntryId(command.journalEntryId);
    if (existingByEntry && existingByEntry.organizationId !== command.organizationId) {
        throwPerformanceError("PERF_CROSS_TENANT", "journal entry organization mismatch");
    }
    const replayByEntry = await loadIdempotentByJournalEntryId(deps.commandJournal, command.journalEntryId);
    if (replayByEntry)
        return replayByEntry;
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        if (racedByCommand) {
            ...
            return performanceCommandResultSchema.parse({ ...parsed, idempotentReplay: true });
        }
        const racedByEntry = await ctx.commandJournal.findByJournalEntryId(command.journalEntryId);
        if (racedByEntry) {
        ...
        const existingSnapshot = await ctx.outcomeSnapshots.findByJournalEntryId(command.journalEntryId);
        if (existingSnapshot) {
            if (existingSnapshot.organizationId !== command.organizationId) {
                throwPerformanceError("PERF_CROSS_TENANT", "outcome snapshot organization mismatch");
            }
