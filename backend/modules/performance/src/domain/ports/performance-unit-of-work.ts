...
export interface OutcomeSnapshotRepository {
    ...
    findByJournalEntryId(journalEntryId: string): Promise<OutcomeSnapshotRecord | null>;
...
export interface PerformanceUnitOfWork {
    runInTransaction<T>(work: (ctx: PerformanceTransactionContext) => Promise<T>): Promise<T>;
}
