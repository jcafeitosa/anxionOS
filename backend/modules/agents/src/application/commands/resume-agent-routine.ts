...
export async function resumeAgentRoutine(deps: ResumeAgentRoutineDeps, input: ResumeAgentRoutineInput): Promise<CommandResult> {
	...
	return deps.unitOfWork.runInTransaction(buildOrganizationTenantContext(pre.organizationId), async (context) => {
		...
		const updated = await context.agentRoutineRepository.save({ ...routine, status: "active", revision, updatedAt: now });
		const result = commandResultSchema.parse({ aggregateId: updated.id, revision });
		await context.commandJournal.record({ tenantId: pre.organizationId, commandId: command.commandId, commandName: "ResumeAgentRoutine", aggregateId: updated.id, aggregateType: "AgentRoutine", revision, responseSnapshot: toCommandResultSnapshot(result) });
		await context.publishEvents([createAgentRoutineResumedEvent({ routineId: updated.id, agentId: updated.agentId, organizationId: updated.organizationId, fromStatus: "paused", toStatus: "active", revision })]);
		return result;
	});
}

export interface ResumeAgentRoutineInput extends ResumeAgentRoutineCommand { actorPrincipalId?: string; }
export interface ResumeAgentRoutineDeps { unitOfWork: AgentsUnitOfWork; commandJournal: CommandJournalRepository; agentRoutineRepository: AgentRoutineRepository; }
