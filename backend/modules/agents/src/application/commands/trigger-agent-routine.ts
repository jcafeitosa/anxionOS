...
function budgetAllows(policy: {
	caps: { wakeupUnitCap: number; tokenUnitCap: number; timeSecondsCap: number };
...
export async function triggerAgentRoutine(
	deps: TriggerAgentRoutineDeps,
	...
	const replayRaw = await deps.commandJournal.findByCommandId(
		pre.organizationId,
		command.commandId,
	);
	...
	return deps.unitOfWork.runInTransaction(
		...
		async (context) => {
			...
			if (routine.status === "paused")
				throwAgentsError(
					"AGT_ROUTINE_PAUSED",
					`Routine is paused: ${command.routineId}`,
				);
			if (routine.lastDedupeKey === command.dedupeKey && routine.lastRunId) {
				const result = triggerAgentRoutineResultSchema.parse({
					routineId: routine.id,
					revision: routine.revision,
					dedupeKey: command.dedupeKey,
					runId: routine.lastRunId,
					idempotentReplay: true,
				});
				await context.commandJournal.record({
					tenantId: pre.organizationId,
					commandId: command.commandId,
					commandName: "TriggerAgentRoutine",
			...
			const budget = await context.agentBudgetRepository.findByAgentId(
				routine.agentId,
			);
			if (!budget || !budgetAllows(budget))
				throwAgentsError(
					"AGT_BUDGET_EXHAUSTED",
					`Agent budget exhausted for agent: ${routine.agentId}`,
				);
			const runId = deps.runDispatch
				? (
						await deps.runDispatch.dispatchRun({
			...
			const updated = await context.agentRoutineRepository.save({
				...routine,
				lastDedupeKey: command.dedupeKey,
				lastRunId: runId,
				lastTriggeredAt: now,
				revision,
				updatedAt: now,
...
export interface TriggerAgentRoutineInput extends TriggerAgentRoutineCommand {
