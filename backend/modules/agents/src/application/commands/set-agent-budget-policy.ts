...
export async function setAgentBudgetPolicy(deps: SetAgentBudgetPolicyDeps, input: SetAgentBudgetPolicyInput): Promise<CommandResult> {
	...
	const existing = await deps.agentBudgetRepository.findByAgentId(command.agentId);
	...
	return deps.unitOfWork.runInTransaction(buildOrganizationTenantContext(agent.organizationId, { agencyId: agent.agencyId }), async (context) => {
		...
		await context.commandJournal.record({ tenantId: agent.organizationId, commandId: command.commandId, commandName: "SetAgentBudgetPolicy", aggregateId: saved.id, aggregateType: "AgentBudgetPolicy", revision, responseSnapshot: toCommandResultSnapshot(result) });
		await context.publishEvents([createAgentBudgetPolicySetEvent({ agentId: saved.agentId, organizationId: saved.organizationId, caps: saved.caps, status: saved.status, revision })]);
		return result;
	});
}

export interface SetAgentBudgetPolicyInput extends SetAgentBudgetPolicyCommand { actorPrincipalId?: string; }
export interface SetAgentBudgetPolicyDeps { unitOfWork: AgentsUnitOfWork; commandJournal: CommandJournalRepository; agentRepository: AgentRepository; agentBudgetRepository: AgentBudgetRepository; }
