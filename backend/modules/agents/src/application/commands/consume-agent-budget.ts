...
export async function consumeAgentBudget(
	deps: ConsumeAgentBudgetDeps,
	...
	if (replay?.aggregateId) {
		...
		if (policy)
			return consumeAgentBudgetResultSchema.parse({
		...
		async (context) => {
			...
			if (policy.revision !== command.expectedRevision)
				throwAgentsError(
					...
					`Expected budget revision ${command.expectedRevision}, found ${policy.revision}`,
				);
			if (policy.status === "exhausted")
				throwAgentsError(
					"AGT_BUDGET_EXHAUSTED",
					`Agent budget exhausted: ${command.agentId}`,
				);
			const now = new Date();
			const next = {
				...policy,
			...
			if (exhausted) {
				...
				if (pausedAgent && pausedAgent.status !== "PAUSED") {
					await context.agentRepository.save({
						...pausedAgent,
						status: "PAUSED",
						revision: pausedAgent.revision + 1,
						updatedAt: now,
					});
				}
				events.push(
					createAgentBudgetExhaustedEvent({
			...
			const result = consumeAgentBudgetResultSchema.parse({
				agentId: saved.agentId,
			...
			await context.commandJournal.record({
				...
				commandName: "ConsumeAgentBudget",
...
export interface ConsumeAgentBudgetInput extends ConsumeAgentBudgetCommand {
	actorPrincipalId?: string;
}
export interface ConsumeAgentBudgetDeps {
