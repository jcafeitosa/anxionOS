...
import { agentBudgetPolicies, type AgentBudgetPolicyRow } from "./schema";

export function toAgentBudgetPolicy(row: AgentBudgetPolicyRow): AgentBudgetPolicy {
	return {
		id: row.id,
...
export function createDrizzleAgentBudgetRepository(
	...
	return {
		async save(policy) {
			const existing = await db.select().from(agentBudgetPolicies).where(eq(agentBudgetPolicies.id, policy.id)).limit(1);
			if (existing[0]) {
				const rows = await db.update(agentBudgetPolicies).set({
				...
				}).where(eq(agentBudgetPolicies.id, policy.id)).returning();
				const row = rows[0];
				if (!row) throw new Error("Failed to update agent budget policy");
				return toAgentBudgetPolicy(row);
			}
			const rows = await db.insert(agentBudgetPolicies).values({
				id: policy.id,
		...
		async findByAgentId(agentId) {
			const rows = await db.select().from(agentBudgetPolicies).where(eq(agentBudgetPolicies.agentId, agentId)).limit(1);
