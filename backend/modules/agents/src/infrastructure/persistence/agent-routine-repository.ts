...
import { agentRoutines, type AgentRoutineRow } from "./schema";
...
export function createDrizzleAgentRoutineRepository(
	...
	return {
		async save(routine) {
			const existing = await db.select().from(agentRoutines).where(eq(agentRoutines.id, routine.id)).limit(1);
			if (existing[0]) {
				const rows = await db.update(agentRoutines).set({
				...
				}).where(eq(agentRoutines.id, routine.id)).returning();
				const row = rows[0];
				if (!row) throw new Error("Failed to update agent routine");
				return toAgentRoutine(row);
			}
			const rows = await db.insert(agentRoutines).values({
				id: routine.id,
		...
		async findById(routineId) {
			const rows = await db.select().from(agentRoutines).where(eq(agentRoutines.id, routineId)).limit(1);
			return rows[0] ? toAgentRoutine(rows[0]) : null;
		},
		async findByAgentAndSlug(agentId, slug) {
			const rows = await db.select().from(agentRoutines).where(and(eq(agentRoutines.agentId, agentId), eq(agentRoutines.slug, slug))).limit(1);
			return rows[0] ? toAgentRoutine(rows[0]) : null;
		},
	};
}
