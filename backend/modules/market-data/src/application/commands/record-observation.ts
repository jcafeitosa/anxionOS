import { type MarketDataCommandResult, type RecordObservationCommand } from "@anxionos/contracts/market-data";
import type { CommandJournalRepository } from "../../domain/ports/command-journal";
import type { MarketDataUnitOfWork } from "../../domain/ports/market-data-unit-of-work";
import { randomUUID } from "node:crypto";
import { marketDataCommandResultSchema, recordObservationCommandSchema } from "@anxionos/contracts/market-data";
...
export async function recordObservation(deps: RecordObservationDeps, input: RecordObservationCommand): Promise<MarketDataCommandResult> {
    ...
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        if (raced) {
            ...
            return marketDataCommandResultSchema.parse({ ...parsed, idempotentReplay: true });
        }
        const instrument = await ctx.instruments.findById(command.instrumentId, command.organizationId);
        if (!instrument || instrument.status !== "ACTIVE") {
            throwMarketDataError("MD_INSTRUMENT_NOT_FOUND", `Instrument ${command.instrumentId} not found`);
        }
        const dup = await ctx.observations.findBySourceEventId(command.organizationId, command.sourceEventId);
        if (dup) {
