	...
	return {
		async findByCommandId(organizationId, commandId) {
			...
			return {
				commandId,
				...entry,
				responseSnapshot: { aggregateId: "md_ins_1", revision: 1 },
			};
		},
		async save() {},
	};
...
describe("market-data command intent", () => {
	...
	test("legacy journal row without hash fails closed", async () => {
		const intent = createMarketDataCommandIntent("registerInstrument", {
