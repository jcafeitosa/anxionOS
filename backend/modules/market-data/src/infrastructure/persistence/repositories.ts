...
export function createPgInstrumentRepository(client: PoolClient): InstrumentRepository {
    return {
        async findById(instrumentId, organizationId) {
            const result = await client.query(`SELECT * FROM market_data_instruments WHERE id = $1 AND organization_id = $2`, [instrumentId, organizationId]);
            const row = result.rows[0];
            return row ? mapInstrument(row) : null;
        },
        async findActiveByNaturalKey(organizationId, canonicalSymbol, venueId) {
            const result = await client.query(`SELECT * FROM market_data_instruments
				 WHERE organization_id = $1 AND canonical_symbol = $2 AND venue_id = $3 AND status = 'ACTIVE'`, [organizationId, canonicalSymbol, venueId]);
            const row = result.rows[0];
            return row ? mapInstrument(row) : null;
        },
        async save(record: InstrumentRecord) {
            await client.query(`INSERT INTO market_data_instruments (
...
export function createPgObservationRepository(client: PoolClient): ObservationRepository {
    return {
        async findBySourceEventId(organizationId, sourceEventId) {
            const result = await client.query(`SELECT * FROM market_data_observation_headers
				 WHERE organization_id = $1 AND source_event_id = $2`, [organizationId, sourceEventId]);
            const row = result.rows[0];
            if (!row)
                return null;
            return {
        ...
        async saveHeader(record: ObservationHeaderRecord) {
            await client.query(`INSERT INTO market_data_observation_headers (
        ...
        volume: string | null;
            ...
            await client.query(`INSERT INTO market_data_observations_ts (
