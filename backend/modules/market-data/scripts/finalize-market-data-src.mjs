...
write(
	"src/domain/events/market-data-events.ts",
...
write(
	"src/infrastructure/market-data-unit-of-work.ts",
	`import type { DomainEventEnvelope } from "@anxionos/contracts/events";
...
for (const [rel, fn, depsType, cmdType] of [
	[
		"src/application/commands/register-instrument.ts",
		"registerInstrument",
		"RegisterInstrumentDeps",
		"RegisterInstrumentCommand",
	],
	[
		"src/application/commands/record-observation.ts",
		"recordObservation",
...
function mapInstrument`,
		...
		.replace(
			/canonicalSymbol: row\.canonical_symbol,/g,
			"canonicalSymbol: String(row.canonical_symbol),",
		)
		.replace(
			/instrumentKind: row\.instrument_kind,/g,
		...
		.replace(
			/observationKind: row\.observation_kind,/g,
		...
		.replace(
			/eventTime: row\.event_time\.toISOString\(\)/g,
			"eventTime: (row.event_time as Date).toISOString()",
		)
		.replace(
			/qualityFlag: row\.quality_flag,/g,
			"qualityFlag: String(row.quality_flag),",
		)
		.replace(
			/async saveHeader\(record\)/g,
