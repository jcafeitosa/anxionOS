...
function fixImports(content) {
	let out = content.replace(/import type \{([^}]*)\} from "@anxionos\/contracts";/g, (_m, specifiers) => {
		...
		if (marketData.length) parts.push(`import type { ${marketData.join(", ")} } from "@anxionos/contracts/market-data";`);
		return parts.join("\n");
	});
	out = out.replace(/import\s*\{([^}]*)\}\s*from\s*"@anxionos\/contracts";/g, (_match, specifiers) => {
		...
		if (marketData.length) parts.push(`import { ${marketData.join(", ")} } from "@anxionos/contracts/market-data";`);
		return parts.join("\n");
	});
