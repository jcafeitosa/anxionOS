...
function fixImports(content) {
	let out = content.replace(
		...
		(_m, specifiers) => {
			const { events, marketData } = splitContractImports(specifiers);
			const parts = [];
			if (events.length)
				parts.push(
			...
			if (marketData.length)
				parts.push(
					`import type { ${marketData.join(", ")} } from "@anxionos/contracts/market-data";`,
				);
			return parts.join("\n");
		},
	);
	out = out.replace(
		...
		(_match, specifiers) => {
			...
			if (marketData.length)
				parts.push(
					`import { ${marketData.join(", ")} } from "@anxionos/contracts/market-data";`,
				);
			return parts.join("\n");
		},
	);
