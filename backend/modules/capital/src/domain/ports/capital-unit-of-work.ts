...
export interface CapitalAccountRepository {
    findById(accountId: string, organizationId: string): Promise<CapitalAccountRecord | null>;
    findActiveByNaturalKey(organizationId: string, ownerUserId: string): Promise<CapitalAccountRecord | null>;
    save(record: CapitalAccountRecord): Promise<CapitalAccountRecord>;
}
export interface BalanceLineRepository {
    lockForUpdate(accountId: string, asset: string): Promise<BalanceLineRecord | null>;
    ...
    assertAvailableForReservation(accountId: string, asset: string, amount: string): Promise<BalanceLineRecord>;
}
export interface AllocationRepository {
    save(record: AllocationRecord): Promise<AllocationRecord>;
}
export interface ReservationRepository {
    findActiveByIntent(accountId: string, intentHash: string): Promise<ReservationRecord | null>;
...
export interface CapitalUnitOfWork {
    runInTransaction<T>(work: (ctx: CapitalTransactionContext) => Promise<T>): Promise<T>;
}
