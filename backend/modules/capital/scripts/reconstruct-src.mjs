...
function fixImports(content) {
	let out = content.replace(/import type \{([^}]*)\} from "@anxionos\/contracts";/g, (_m, specifiers) => {
		...
		if (capital.length) parts.push(`import type { ${capital.join(", ")} } from "@anxionos/contracts/capital";`);
		return parts.join("\n");
	});
	out = out.replace(/import\s*\{([^}]*)\}\s*from\s*"@anxionos\/contracts";/g, (_match, specifiers) => {
		...
		if (capital.length) parts.push(`import { ${capital.join(", ")} } from "@anxionos/contracts/capital";`);
		return parts.join("\n");
	});
