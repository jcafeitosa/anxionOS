...
import { canTransitionMembershipStatus, wouldViolateOwnerRequired } from "../../domain/entities/membership";
...
export async function revokeMembership(
	...
	return deps.unitOfWork.runInTransaction(
		...
		const membership = await context.membershipRepository.findById(command.agencyId, command.membershipId);
		if (!membership) {
		...
		const agencyMemberships = await context.membershipRepository.listByAgency(command.agencyId);
		if (wouldViolateOwnerRequired(agencyMemberships, command.membershipId)) {
			throwOrganizationError("ORG_OWNER_REQUIRED", "Cannot revoke the sole active owner of the agency");
		}
		const now = new Date();
