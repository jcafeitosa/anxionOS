...
export async function activateMembership(
	...
	return deps.unitOfWork.runInTransaction(async (context) => {
		...
		if (membership.status !== "invited") {
			throwOrganizationError("ORG_MEMBERSHIP_NOT_INVITED", `Membership ${command.membershipId} is not invited`);
		}
		if (membership.inviteExpiresAt && membership.inviteExpiresAt.getTime() <= Date.now()) {
			throwOrganizationError("ORG_INVITE_EXPIRED", `Invite for membership ${command.membershipId} has expired`);
		}
