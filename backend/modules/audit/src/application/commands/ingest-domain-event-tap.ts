...
export async function ingestDomainEventTap(
	deps: IngestDomainEventTapDeps,
	input: IngestDomainEventTapCommand,
	...
	const existingBySource = await deps.commandJournal.findBySourceEventId(
		command.sourceEventId,
	);
	if (
		existingBySource &&
		existingBySource.organizationId !== command.organizationId
	) {
		throwAuditError("AUD_CROSS_TENANT", "source event organization mismatch");
	}
	const replayBySource = await loadIdempotentBySourceEventId(
		deps.commandJournal,
		command.sourceEventId,
	);
	if (replayBySource) return replayBySource;
	return deps.unitOfWork.runInTransaction(async (ctx) => {
		...
		const racedBySource = await ctx.commandJournal.findBySourceEventId(
			command.sourceEventId,
		);
		if (racedBySource) {
			if (racedBySource.organizationId !== command.organizationId) {
				throwAuditError(
					"AUD_CROSS_TENANT",
					"source event organization mismatch",
				);
			}
		...
		const existingManifest = await ctx.manifests.findBySourceEventId(
			command.sourceEventId,
		);
		if (existingManifest) {
