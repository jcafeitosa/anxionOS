...
export function createPgAuditManifestRepository(client: PoolClient): AuditManifestRepository {
    return {
        async findById(id) {
            const result = await client.query(`SELECT * FROM audit_manifests WHERE id = $1`, [id]);
            const row = result.rows[0];
            return row ? mapManifest(row) : null;
        },
        async findBySourceEventId(sourceEventId) {
            const result = await client.query(`SELECT * FROM audit_manifests WHERE source_event_id = $1`, [sourceEventId]);
            const row = result.rows[0];
            return row ? mapManifest(row) : null;
        },
