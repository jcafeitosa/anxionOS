...
write("src/application/command-support.ts", `import type { AuditCommandResult } from "@anxionos/contracts/audit";
...
write("src/domain/events/audit-events.ts", `import { randomUUID } from "node:crypto";
...
write("src/infrastructure/audit-unit-of-work.ts", `import type { DomainEventEnvelope } from "@anxionos/contracts/events";
...
for (const [rel, fn, depsType, cmdType] of [
  ["src/application/commands/ingest-domain-event-tap.ts", "ingestDomainEventTap", "IngestDomainEventTapDeps", "IngestDomainEventTapCommand"],
]) {
  let c = read(rel);
...
write("src/application/consumers/domain-event-tap-consumer.ts", `import { randomUUID } from "node:crypto";
import {
...
function mapManifest`,
    ...
    .replace("function mapManifest(row) {", "function mapManifest(row: Record<string, unknown>): AuditManifestRecord {")
    ...
    .replace(/ownerDomain: row\.owner_domain,/g, "ownerDomain: String(row.owner_domain),")
    .replace(/eventType: row\.event_type,/g, "eventType: String(row.event_type),")
    .replace(/occurredAt: row\.occurred_at\.toISOString\(\)/g, "occurredAt: (row.occurred_at as Date).toISOString()")
    .replace(/payloadHash: row\.payload_hash,/g, "payloadHash: String(row.payload_hash),")
