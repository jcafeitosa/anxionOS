...
function fixImports(content) {
	let out = content.replace(/import type \{([^}]*)\} from "@anxionos\/contracts";/g, (_m, specifiers) => {
		...
		if (audit.length) parts.push(`import type { ${audit.join(", ")} } from "@anxionos/contracts/audit";`);
		return parts.join("\n");
	});
	out = out.replace(/import\s*\{([^}]*)\}\s*from\s*"@anxionos\/contracts";/g, (_match, specifiers) => {
		...
		if (audit.length) parts.push(`import { ${audit.join(", ")} } from "@anxionos/contracts/audit";`);
		return parts.join("\n");
	});
