...
const DEFAULT_CHART_ACCOUNTS = [
	...
	{ code: "trading.fees", kind: "EXPENSE" },
];
...
export function createPgChartAccountRepository(
	client: PoolClient,
): ChartAccountRepository {
	return {
		async findByCode(organizationId: string, code: string) {
			const result = await client.query(
				"SELECT * FROM accounting_chart_accounts WHERE organization_id = $1 AND code = $2",
				[organizationId, code],
			);
		...
		async ensureDefaultChart(organizationId: string, currency: string) {
			for (const account of DEFAULT_CHART_ACCOUNTS) {
				await client.query(
					`INSERT INTO accounting_chart_accounts (id, organization_id, code, kind, currency, status)
					 VALUES ($1,$2,$3,$4,$5,'ACTIVE')
					 ON CONFLICT (organization_id, code) DO NOTHING`,
					[
						`acc_chart_${randomUUID()}`,
						organizationId,
						account.code,
						account.kind,
						currency,
...
export function createPgJournalEntryRepository(
	client: PoolClient,
): JournalEntryRepository {
	return {
		async findById(id: string) {
			const result = await client.query(
				"SELECT * FROM accounting_journal_entries WHERE id = $1",
				[id],
			);
			const row = result.rows[0];
...
export function createPgLedgerPostingRepository(
	client: PoolClient,
): LedgerPostingRepository {
	return {
		async save(record: LedgerPostingRecord) {
			await client.query(
				`INSERT INTO accounting_ledger_postings (
		...
		async findByEntryId(journalEntryId: string) {
			const result = await client.query(
				"SELECT * FROM accounting_ledger_postings WHERE journal_entry_id = $1 ORDER BY created_at",
				[journalEntryId],
			);
