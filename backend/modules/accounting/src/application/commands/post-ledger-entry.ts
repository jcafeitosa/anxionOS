...
async function assertAccountsBelongToOrg(
	...
	for (const line of lines) {
		const account = await ctx.chartAccounts.findByCode(
			organizationId,
			line.accountCode,
		);
		if (!account) {
			throwAccountingError(
				...
				`chart account not found: ${line.accountCode}`,
			);
		}
...
function entryResultFromRecord(
...
export async function postLedgerEntry(
	deps: PostLedgerEntryDeps,
	input: PostLedgerEntryCommand,
	...
	return deps.unitOfWork.runInTransaction(async (ctx) => {
		...
		const existing = await ctx.journalEntries.findByIdempotencyKey(
		...
		await ctx.chartAccounts.ensureDefaultChart(
			command.organizationId,
			settlementAsset,
		);
		...
		const valueDate =
			command.valueDate ?? new Date().toISOString().slice(0, 10);
