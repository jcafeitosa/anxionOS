...
export function assertBalancedLines(lines: LedgerLine[]): void {
	if (lines.length < 2) {
		throwAccountingError(
			"ACC_UNBALANCED_ENTRY",
			"ledger entry requires at least two lines",
		);
	}
	...
	for (const line of lines) {
		...
		if (debit === 0 && credit === 0) {
			throwAccountingError(
				"ACC_UNBALANCED_ENTRY",
				"line must have debit or credit",
			);
		}
		...
		if (Math.abs(active - amount) > 0.00000001) {
			throwAccountingError(
				"ACC_UNBALANCED_ENTRY",
				"amount must match debit or credit",
			);
		}
	...
	for (const [asset, totals] of totalsByAsset) {
		if (Math.abs(totals.debit - totals.credit) > 0.00000001) {
			throwAccountingError(
				"ACC_UNBALANCED_ENTRY",
				`unbalanced entry for asset ${asset}: debit ${totals.debit} credit ${totals.credit}`,
			);
		}
	}
}
