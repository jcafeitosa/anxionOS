...
export interface ChartAccountRepository {
    findByCode(organizationId: string, code: string): Promise<ChartAccountRecord | null>;
    ensureDefaultChart(organizationId: string, currency: string): Promise<void>;
}
export interface JournalEntryRepository {
    findByIdempotencyKey(organizationId: string, idempotencyKey: string): Promise<JournalEntryRecord | null>;
...
export interface AccountingUnitOfWork {
    runInTransaction<T>(work: (ctx: AccountingTransactionContext) => Promise<T>): Promise<T>;
}
