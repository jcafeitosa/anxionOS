...
function fixImports(content) {
	let out = content.replace(/import type \{([^}]*)\} from "@anxionos\/contracts";/g, (_m, specifiers) => {
		...
		if (knowledge.length) parts.push(`import type { ${knowledge.join(", ")} } from "@anxionos/contracts/knowledge";`);
		return parts.join("\n");
	});
	out = out.replace(/import\s*\{([^}]*)\}\s*from\s*"@anxionos\/contracts";/g, (_match, specifiers) => {
		...
		if (knowledge.length) parts.push(`import { ${knowledge.join(", ")} } from "@anxionos/contracts/knowledge";`);
		return parts.join("\n");
	});
