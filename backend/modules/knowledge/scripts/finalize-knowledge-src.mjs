...
write(
	"src/application/text-chunking.ts",
...
write(
	"src/infrastructure/adapters/simulated-embedding-adapter.ts",
	`import { createHash } from "node:crypto";
...
write(
	"src/infrastructure/knowledge-unit-of-work.ts",
...
write(
	...
	read("dist/infrastructure/persistence/repositories.js")
		...
		.replace(
			/displayName: row\.display_name,/g,
			"displayName: String(row.display_name),",
		)
		.replace(
			/sourceKind: row\.source_kind,/g,
			"sourceKind: String(row.source_kind),",
		)
		.replace(
			/defaultClassification: row\.default_classification,/g,
			"defaultClassification: String(row.default_classification),",
		)
		.replace(
			/defaultAclId: row\.default_acl_id,/g,
			"defaultAclId: String(row.default_acl_id),",
		)
		.replace(
			/defaultAclEpoch: row\.default_acl_epoch,/g,
		...
		.replace(
			"export function createPgKnowledgeSourceRepository(client) {",
			"export function createPgKnowledgeSourceRepository(client: PoolClient): KnowledgeSourceRepository {",
		)
		.replace(
			"export function createPgDocumentRepository(client) {",
			"export function createPgDocumentRepository(client: PoolClient): DocumentRepository {",
		)
		.replace(
			"export function createPgDocumentVersionRepository(client) {",
			"export function createPgDocumentVersionRepository(client: PoolClient): DocumentVersionRepository {",
		)
		.replace(
			"export function createPgIndexGenerationRepository(client) {",
			"export function createPgIndexGenerationRepository(client: PoolClient): IndexGenerationRepository {",
		)
		.replace(
			"export function createPgChunkRepository(client) {",
			"export function createPgChunkRepository(client: PoolClient): ChunkRepository {",
		)
		.replace(
			"export function createPgEmbeddingRepository(client) {",
			"export function createPgEmbeddingRepository(client: PoolClient): EmbeddingRepository {",
		)
		.replace(
			"export function createPgEmbeddingSpaceRepository(client) {",
			"export function createPgEmbeddingSpaceRepository(client: PoolClient): EmbeddingSpaceRepository {",
		)
		.replace(
			/knowledgeSourceId: row\.knowledge_source_id,/g,
			"knowledgeSourceId: String(row.knowledge_source_id),",
		)
		.replace(/title: row\.title,/g, "title: String(row.title),")
		.replace(
			/classification: row\.classification,/g,
			"classification: String(row.classification),",
		)
		.replace(/aclId: row\.acl_id,/g, "aclId: String(row.acl_id),")
		.replace(/aclEpoch: row\.acl_epoch,/g, "aclEpoch: Number(row.acl_epoch),")
		.replace(
			/activeVersionId: row\.active_version_id,/g,
			"activeVersionId: row.active_version_id ? String(row.active_version_id) : null,",
		)
		.replace(
			/documentId: row\.document_id,/g,
			"documentId: String(row.document_id),",
		)
		.replace(
			/versionNumber: row\.version_number,/g,
			"versionNumber: Number(row.version_number),",
		)
		.replace(
			/contentHash: row\.content_hash,/g,
			"contentHash: String(row.content_hash),",
		)
		.replace(
			/blobBucket: row\.blob_bucket,/g,
			"blobBucket: String(row.blob_bucket),",
		)
		.replace(
			/blobObjectKey: row\.blob_object_key,/g,
		...
		.replace(
			"export function splitIntoChunks(text, maxLen = 256) {",
			"export function splitIntoChunks(text: string, maxLen = 256): string[] {",
		)
		.replace(
		...
		.replace(
			/async save\(record\)/g,
			"async save(record: KnowledgeSourceRecord | DocumentRecord | DocumentVersionRecord | IndexGenerationRecord | EmbeddingSpaceRecord | { id: string; organizationId: string; chunkId: string; embeddingSpaceId: string; dimensions: number; vector: number[] })",
		),
);
