import { type KnowledgeCommandResult, type PublishIndexCommand } from "@anxionos/contracts/knowledge";
import type { CommandJournalRepository } from "../../domain/ports/command-journal";
import type { KnowledgeUnitOfWork } from "../../domain/ports/knowledge-unit-of-work";
import { knowledgeCommandResultSchema, publishIndexCommandSchema } from "@anxionos/contracts/knowledge";
...
export async function publishIndex(deps: PublishIndexDeps, input: PublishIndexCommand): Promise<KnowledgeCommandResult> {
    ...
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        if (raced) {
            ...
            return knowledgeCommandResultSchema.parse({ ...parsed, idempotentReplay: true });
        }
        const document = await ctx.documents.findById(command.documentId, command.organizationId);
        if (!document) {
            throwKnowledgeError("KN_DOCUMENT_NOT_FOUND", `Document ${command.documentId} not found`);
        }
        if (document.revision !== command.expectedRevision) {
            throwKnowledgeError("KN_REVISION_CONFLICT", "Document revision mismatch");
        }
        const index = await ctx.indexGenerations.findById(command.indexGenerationId, command.organizationId);
        if (!index || index.documentId !== command.documentId) {
            throwKnowledgeError("KN_INDEX_NOT_FOUND", `Index ${command.indexGenerationId} not found`);
        }
        if (index.status !== "READY" || index.embeddedCount < index.chunkCount) {
            throwKnowledgeError("KN_INDEX_NOT_READY", "Index generation is not ready for publish");
        }
        const embeddedCount = await ctx.embeddings.countByIndexGeneration(index.id);
        if (embeddedCount < index.chunkCount) {
            throwKnowledgeError("KN_INDEX_NOT_READY", "Embeddings incomplete — activeVersion unchanged");
        }
        ...
        const version = await ctx.documentVersions.findById(index.documentVersionId, command.organizationId);
        if (!version) {
            throwKnowledgeError("KN_DOCUMENT_NOT_FOUND", "Document version missing for publish");
        }
