import { type KnowledgeCommandResult, type RegisterKnowledgeSourceCommand } from "@anxionos/contracts/knowledge";
import type { CommandJournalRepository } from "../../domain/ports/command-journal";
import type { KnowledgeUnitOfWork } from "../../domain/ports/knowledge-unit-of-work";
import { randomUUID } from "node:crypto";
import { knowledgeCommandResultSchema, registerKnowledgeSourceCommandSchema } from "@anxionos/contracts/knowledge";
...
export async function registerKnowledgeSource(deps: RegisterKnowledgeSourceDeps, input: RegisterKnowledgeSourceCommand): Promise<KnowledgeCommandResult> {
    ...
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        const existing = await ctx.sources.findActiveByNaturalKey(command.organizationId, command.displayName);
        if (existing) {
