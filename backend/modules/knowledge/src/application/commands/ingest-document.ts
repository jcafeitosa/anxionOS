import { type IngestDocumentCommand, type KnowledgeCommandResult } from "@anxionos/contracts/knowledge";
...
export async function ingestDocument(deps: IngestDocumentDeps, input: IngestDocumentCommand): Promise<KnowledgeCommandResult> {
    ...
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        const source = await ctx.sources.findById(command.knowledgeSourceId, command.organizationId);
        if (!source || source.status !== "ACTIVE") {
            throwKnowledgeError("KN_SOURCE_NOT_FOUND", `Knowledge source ${command.knowledgeSourceId} not found or inactive`);
        }
        let space = await ctx.embeddingSpaces.findById(command.embeddingSpaceId, command.organizationId);
        if (!space) {
        ...
        const embeddings = await deps.embeddingPort.embedBatch(chunkRecords.map((chunk, idx) => ({
            chunkId: chunk.id,
            text: textChunks[idx],
        ...
        for (const embedding of embeddings) {
            ...
            events.push(createChunkEmbeddedEvent({
                chunkId: embedding.chunkId,
                documentVersionId: versionId,
                embeddingSpaceId: command.embeddingSpaceId,
                dimensions: embedding.dimensions,
