...
export interface KnowledgeSourceRepository {
    findById(sourceId: string, organizationId: string): Promise<KnowledgeSourceRecord | null>;
    findActiveByNaturalKey(organizationId: string, displayName: string): Promise<KnowledgeSourceRecord | null>;
    save(record: KnowledgeSourceRecord): Promise<KnowledgeSourceRecord>;
}
export interface DocumentRepository {
    findById(documentId: string, organizationId: string): Promise<DocumentRecord | null>;
...
export interface DocumentVersionRepository {
    findById(id: string, organizationId: string): Promise<DocumentVersionRecord | null>;
...
export interface IndexGenerationRepository {
    findById(id: string, organizationId: string): Promise<IndexGenerationRecord | null>;
...
export interface EmbeddingSpaceRepository {
    findById(id: string, organizationId: string): Promise<EmbeddingSpaceRecord | null>;
...
export interface KnowledgeUnitOfWork {
    runInTransaction<T>(work: (ctx: KnowledgeTransactionContext) => Promise<T>): Promise<T>;
}
