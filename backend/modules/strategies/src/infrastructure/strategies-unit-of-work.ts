...
function createTransactionContext(client: PoolClient): StrategiesTransactionContext {
  return {
...
export function createStrategiesUnitOfWork(pool: Pool): StrategiesUnitOfWork {
  return {
    async runInTransaction<T>(work: (ctx: StrategiesTransactionContext) => Promise<T>) {
