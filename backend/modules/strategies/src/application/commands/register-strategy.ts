import { type RegisterStrategyCommand, type StrategiesCommandResult } from "@anxionos/contracts/strategies";
import type { CommandJournalRepository } from "../../domain/ports/command-journal";
import type { StrategiesUnitOfWork } from "../../domain/ports/strategies-unit-of-work";
import { randomUUID } from "node:crypto";
import { registerStrategyCommandSchema, strategiesCommandResultSchema } from "@anxionos/contracts/strategies";
...
export async function registerStrategy(deps: RegisterStrategyDeps, input: RegisterStrategyCommand): Promise<StrategiesCommandResult> {
    ...
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        const existing = await ctx.strategies.findActiveByNaturalKey(command.organizationId, command.displayName);
        if (existing) {
