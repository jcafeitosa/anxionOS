import { type PublishStrategyVersionCommand, type StrategiesCommandResult } from "@anxionos/contracts/strategies";
import type { CommandJournalRepository } from "../../domain/ports/command-journal";
import type { StrategiesUnitOfWork } from "../../domain/ports/strategies-unit-of-work";
import { assertValidStrategyVersionLifecycleTransition, publishStrategyVersionCommandSchema, strategiesCommandResultSchema } from "@anxionos/contracts/strategies";
...
export async function publishStrategyVersion(deps: PublishStrategyVersionDeps, input: PublishStrategyVersionCommand): Promise<StrategiesCommandResult> {
    ...
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        if (raced) {
            ...
            return strategiesCommandResultSchema.parse({ ...parsed, idempotentReplay: true });
        }
        const strategy = await ctx.strategies.findById(command.strategyId, command.organizationId);
        if (!strategy) {
            throwStrategiesError("ST_STRATEGY_NOT_FOUND", `Strategy ${command.strategyId} not found`);
        }
        const version = await ctx.versions.findById(command.strategyVersionId, command.organizationId);
        if (!version || version.strategyId !== command.strategyId) {
            throwStrategiesError("ST_VERSION_NOT_FOUND", `Strategy version ${command.strategyVersionId} not found`);
        }
        ...
        try {
            ...
            throwStrategiesError("ST_INVALID_LIFECYCLE_TRANSITION", `Cannot publish version in state ${version.lifecycleState}`);
        }
