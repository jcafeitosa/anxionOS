...
export interface StrategyRepository {
  findById(strategyId: string, organizationId: string): Promise<StrategyRecord | null>;
  findActiveByNaturalKey(organizationId: string, displayName: string): Promise<StrategyRecord | null>;
...
export interface StrategyVersionRepository {
  findById(versionId: string, organizationId: string): Promise<StrategyVersionRecord | null>;
...
export interface StrategiesUnitOfWork {
  runInTransaction<T>(work: (ctx: StrategiesTransactionContext) => Promise<T>): Promise<T>;
}
