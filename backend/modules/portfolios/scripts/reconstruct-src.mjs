...
function fixImports(content) {
	let out = content.replace(/import type \{([^}]*)\} from "@anxionos\/contracts";/g, (_m, specifiers) => {
		const { events, accounting, portfolios } = splitContractImports(specifiers);
		const parts = [];
		if (events.length) parts.push(`import type { ${events.join(", ")} } from "@anxionos/contracts/events";`);
		if (accounting.length) parts.push(`import type { ${accounting.join(", ")} } from "@anxionos/contracts/accounting/execution-fill-confirmed-bridge";`);
		if (portfolios.length) parts.push(`import type { ${portfolios.join(", ")} } from "@anxionos/contracts/portfolios";`);
		return parts.join("\n");
	});
	out = out.replace(/import\s*\{([^}]*)\}\s*from\s*"@anxionos\/contracts";/g, (_match, specifiers) => {
		...
		if (accounting.length) parts.push(`import { ${accounting.join(", ")} } from "@anxionos/contracts/accounting/execution-fill-confirmed-bridge";`);
		if (portfolios.length) parts.push(`import { ${portfolios.join(", ")} } from "@anxionos/contracts/portfolios";`);
		return parts.join("\n");
	});
