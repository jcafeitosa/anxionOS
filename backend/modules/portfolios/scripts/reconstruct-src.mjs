...
function parseDeclareFunctions(dts) {
	const regex =
		/export declare (async )?function (\w+)\(([\s\S]*?)\)(:\s*[\s\S]*?)?;/g;
	const out = [];
	let match;
...
function applyFunctionSignatures(js, signatures) {
	...
	for (const sig of signatures) {
		...
		const pattern = new RegExp(
			`export ${asyncPart}function ${sig.name}\\([^)]*\\)\\s*\\{`,
		);
...
function mergeImplFile(dtsPath, jsPath) {
	...
	const headerTypes = header
		...
		.join("\n")
		.trim();
	const jsImports = typedJs
		.split("\n")
		.filter((l) => l.startsWith("import "))
		.join("\n");
	const jsBody = typedJs
		.split("\n")
		.filter((l) => !l.startsWith("import "))
		.join("\n")
		.trim();
...
function splitContractImports(specifiers) {
	...
	for (const name of specifiers
		...
		if (
			bare === "DomainEventEnvelope" ||
			bare === "domainEventEnvelopeSchema"
		) {
			events.push(name);
		} else if (bare === "ExecutionFillConfirmedV1") {
	...
	return { events, accounting, portfolios };
}

function fixImports(content) {
	let out = content.replace(
		/import type \{([^}]*)\} from "@anxionos\/contracts";/g,
		(_m, specifiers) => {
			const { events, accounting, portfolios } =
				splitContractImports(specifiers);
			const parts = [];
			if (events.length)
				parts.push(
					`import type { ${events.join(", ")} } from "@anxionos/contracts/events";`,
				);
			if (accounting.length)
				parts.push(
					`import type { ${accounting.join(", ")} } from "@anxionos/contracts/accounting/execution-fill-confirmed-bridge";`,
				);
			if (portfolios.length)
				parts.push(
					`import type { ${portfolios.join(", ")} } from "@anxionos/contracts/portfolios";`,
				);
			return parts.join("\n");
		},
	);
	out = out.replace(
		/import\s*\{([^}]*)\}\s*from\s*"@anxionos\/contracts";/g,
		(_match, specifiers) => {
			const { events, accounting, portfolios } =
				splitContractImports(specifiers);
			const parts = [];
			if (events.length)
				parts.push(
					`import { ${events.join(", ")} } from "@anxionos/contracts/events";`,
				);
			if (accounting.length)
				parts.push(
					`import { ${accounting.join(", ")} } from "@anxionos/contracts/accounting/execution-fill-confirmed-bridge";`,
				);
			if (portfolios.length)
				parts.push(
					`import { ${portfolios.join(", ")} } from "@anxionos/contracts/portfolios";`,
				);
			return parts.join("\n");
		},
	);
	return out.replace(
		/from "@anxionos\/eventing"/g,
...
function reconstruct() {
	...
	walk(distRoot, (file) => {
		...
		const hasImpl =
			rawDts.includes("declare function") || rawDts.includes("declare class");
		let content;
		if (!hasImpl || !fs.existsSync(jsPath))
			content = `${typeOnlySection(rawDts)}\n`;
