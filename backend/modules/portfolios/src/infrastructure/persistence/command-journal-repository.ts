    ...
    return {
        async findByCommandId(commandId) {
            const result = await client.query(`SELECT command_id, organization_id, command_name, response_snapshot
				 FROM portfolios_command_journal WHERE command_id = $1`, [commandId]);
        ...
        async save(entry) {
            await client.query(`INSERT INTO portfolios_command_journal (command_id, organization_id, command_name, response_snapshot)
