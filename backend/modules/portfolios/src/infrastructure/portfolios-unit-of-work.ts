...
import type { PortfoliosTransactionContext, PortfoliosUnitOfWork } from '../domain/ports/portfolios-unit-of-work';
import { createPgCommandJournalRepository } from './persistence/command-journal-repository';
import { createPgHoldingRepository, createPgPortfolioRepository, createPgPositionRepository } from './persistence/repositories';

function createTransactionContext(client: PoolClient): PortfoliosTransactionContext {
  return {
...
export function createPortfoliosUnitOfWork(pool: Pool): PortfoliosUnitOfWork {
  return {
    async runInTransaction<T>(work: (ctx: PortfoliosTransactionContext) => Promise<T>) {
