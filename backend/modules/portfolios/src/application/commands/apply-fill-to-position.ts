import { type ApplyFillToPositionCommand, type PortfoliosCommandResult } from "@anxionos/contracts/portfolios";
import type { CommandJournalRepository } from "../../domain/ports/command-journal";
import type { PortfoliosUnitOfWork } from "../../domain/ports/portfolios-unit-of-work";
import { randomUUID } from "node:crypto";
import { assertPortfoliosExecutionModeSupported, applyFillToPositionCommandSchema, portfoliosCommandResultSchema } from "@anxionos/contracts/portfolios";
...
function positionResultFromRecord(record: { id: string; revision: number; portfolioId: string }, holdingId: string, idempotentReplay = false) {
...
export async function applyFillToPosition(deps: ApplyFillToPositionDeps, input: ApplyFillToPositionCommand): Promise<PortfoliosCommandResult> {
    ...
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        const existingHolding = await ctx.holdings.findByFillId(command.organizationId, command.fillId);
        if (existingHolding) {
            const position = await ctx.positions.findByPositionKey(command.portfolioId, command.instrumentId, DEFAULT_POSITION_SIDE, DEFAULT_BOOK);
            if (!position) {
                throwPortfoliosError("PF_PORTFOLIO_NOT_FOUND", "position missing for idempotent fill");
            }
            const result = positionResultFromRecord(position, existingHolding.id, true);
        ...
        let position = await ctx.positions.findByPositionKey(command.portfolioId, command.instrumentId, DEFAULT_POSITION_SIDE, DEFAULT_BOOK);
        if (!position) {
            ...
            try {
                ...
                position = await ctx.positions.findByPositionKey(command.portfolioId, command.instrumentId, DEFAULT_POSITION_SIDE, DEFAULT_BOOK);
                if (!position) {
                    throwPortfoliosError("PF_DUPLICATE_POSITION_KEY", "position key conflict");
                }
            }
        }
        const delta = signedDelta(command.side, command.quantity);
        const updatedPosition = await ctx.positions.updateQuantity(position.id, delta, position.revision + 1);
