...
import { mapFillConfirmedToApplyFill, type PortfoliosExecutionFillConfirmedV1 } from '@anxionos/contracts/portfolios';
...
export function createFillConfirmedConsumer(deps: FillConfirmedConsumerDeps) {
  return {
    async handle(fill: PortfoliosExecutionFillConfirmedV1) {
      ...
      return applyFillToPosition({ unitOfWork: deps.unitOfWork, commandJournal: deps.commandJournal }, command);
    },
  };
}
