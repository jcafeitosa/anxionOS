...
export function throwPortfoliosError(code: PortfoliosErrorCode, message: string): never {
  throw new PortfoliosCommandError(code, message);
}

export function parseCommandResultSnapshot(snapshot: Record<string, unknown>): PortfoliosCommandResult {
