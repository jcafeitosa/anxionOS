...
export function createOutcomeRecordedConsumer(deps: OutcomeRecordedConsumerDeps): {
  handle(outcome: PerformanceOutcomeRecordedBridge): Promise<EvaluationCommandResult>;
} {
  return {
    async handle(outcome: PerformanceOutcomeRecordedBridge) {
      const command = mapOutcomeRecordedToEvaluationInput(outcome, randomUUID());
