import { type EvaluationCommandResult, type RecordEvaluationScoreCommand } from "@anxionos/contracts/evaluation";
import type { CommandJournalRepository } from "../../domain/ports/command-journal";
import type { EvaluationUnitOfWork } from "../../domain/ports/evaluation-unit-of-work";
import { randomUUID } from "node:crypto";
import { computeOutcomeNotionalScore, evaluationCommandResultSchema, recordEvaluationScoreCommandSchema } from "@anxionos/contracts/evaluation";
import { createScoreComputedEvent } from "../../domain/events/evaluation-events";
import { loadIdempotentByOutcomeSnapshotId, loadIdempotentCommandResult, toCommandResultSnapshot, } from "../command-support";
...
export async function recordEvaluationScore(deps: RecordEvaluationScoreDeps, input: RecordEvaluationScoreCommand): Promise<EvaluationCommandResult> {
    ...
    if (existingCommand && existingCommand.organizationId !== command.organizationId) {
        throwEvaluationError("EVL_CROSS_TENANT", "command journal organization mismatch");
    }
    const replayByCommand = await loadIdempotentCommandResult(deps.commandJournal, command.commandId);
    if (replayByCommand)
        return replayByCommand;
    const existingByOutcome = await deps.commandJournal.findByOutcomeSnapshotId(command.outcomeSnapshotId);
    if (existingByOutcome && existingByOutcome.organizationId !== command.organizationId) {
        throwEvaluationError("EVL_CROSS_TENANT", "outcome snapshot organization mismatch");
    }
    const replayByOutcome = await loadIdempotentByOutcomeSnapshotId(deps.commandJournal, command.outcomeSnapshotId);
    if (replayByOutcome)
        return replayByOutcome;
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        if (racedByCommand) {
            ...
            return evaluationCommandResultSchema.parse({ ...parsed, idempotentReplay: true });
        }
        const racedByOutcome = await ctx.commandJournal.findByOutcomeSnapshotId(command.outcomeSnapshotId);
        if (racedByOutcome) {
            if (racedByOutcome.organizationId !== command.organizationId) {
                throwEvaluationError("EVL_CROSS_TENANT", "outcome snapshot organization mismatch");
            }
            const parsed = parseCommandResultSnapshot(racedByOutcome.responseSnapshot);
            return evaluationCommandResultSchema.parse({ ...parsed, idempotentReplay: true });
        }
        const existingRecord = await ctx.evaluationRecords.findByOutcomeSnapshotId(command.outcomeSnapshotId);
        if (existingRecord) {
            if (existingRecord.organizationId !== command.organizationId) {
                throwEvaluationError("EVL_CROSS_TENANT", "evaluation record organization mismatch");
            }
            const existingScore = await ctx.evaluationScores.findByEvaluationRecordId(existingRecord.id);
