...
function mapEvaluationRecord(row: Record<string, unknown>): EvaluationRecordRow {
    return {
        id: String(row.id),
...
export function createPgEvaluationRecordRepository(client: PoolClient): EvaluationRecordRepository {
    return {
        async findById(id) {
            const result = await client.query(`SELECT * FROM evaluation_records WHERE id = $1`, [id]);
            const row = result.rows[0];
            return row ? mapEvaluationRecord(row) : null;
        },
        async findByOutcomeSnapshotId(outcomeSnapshotId) {
            const result = await client.query(`SELECT * FROM evaluation_records WHERE outcome_snapshot_id = $1`, [outcomeSnapshotId]);
            const row = result.rows[0];
            return row ? mapEvaluationRecord(row) : null;
        },
        async save(record: EvaluationRecordRow) {
            await client.query(`INSERT INTO evaluation_records (
...
export function createPgEvaluationScoreRepository(client: PoolClient): EvaluationScoreRepository {
    return {
        async findByEvaluationRecordId(evaluationRecordId) {
            const result = await client.query(`SELECT * FROM evaluation_scores WHERE evaluation_record_id = $1`, [evaluationRecordId]);
            const row = result.rows[0];
            return row ? mapEvaluationScore(row) : null;
        },
        async save(record: EvaluationScoreRow) {
            await client.query(`INSERT INTO evaluation_scores (
