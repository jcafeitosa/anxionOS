...
export interface EvaluationRecordRepository {
    ...
    findByOutcomeSnapshotId(outcomeSnapshotId: string): Promise<EvaluationRecordRow | null>;
    save(record: EvaluationRecordRow): Promise<EvaluationRecordRow>;
}
export interface EvaluationScoreRepository {
    findByEvaluationRecordId(evaluationRecordId: string): Promise<EvaluationScoreRow | null>;
...
export interface EvaluationUnitOfWork {
    runInTransaction<T>(work: (ctx: EvaluationTransactionContext) => Promise<T>): Promise<T>;
}
