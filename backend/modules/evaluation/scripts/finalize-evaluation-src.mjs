...
write(
	"src/domain/events/evaluation-events.ts",
...
write(
	"src/infrastructure/evaluation-unit-of-work.ts",
	`import type { DomainEventEnvelope } from "@anxionos/contracts/events";
...
function mapOutcomeSnapshot`,
		...
		.replace(
			/evaluationRecordId: row\.outcome_snapshot_id,/g,
			"evaluationRecordId: String(row.outcome_snapshot_id),",
		)
		.replace(
