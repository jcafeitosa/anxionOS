...
function fixImports(content) {
	let out = content.replace(
		...
		(_m, specifiers) => {
			const { events, evaluation } = splitContractImports(specifiers);
			const parts = [];
			if (events.length)
				parts.push(
			...
			if (evaluation.length)
				parts.push(
					`import type { ${evaluation.join(", ")} } from "@anxionos/contracts/evaluation";`,
				);
			return parts.join("\n");
		},
	);
	out = out.replace(
		...
		(_match, specifiers) => {
			...
			if (evaluation.length)
				parts.push(
					`import { ${evaluation.join(", ")} } from "@anxionos/contracts/evaluation";`,
				);
			return parts.join("\n");
		},
	);
