...
function fixImports(content) {
	let out = content.replace(/import type \{([^}]*)\} from "@anxionos\/contracts";/g, (_m, specifiers) => {
		...
		if (evaluation.length) parts.push(`import type { ${evaluation.join(", ")} } from "@anxionos/contracts/evaluation";`);
		return parts.join("\n");
	});
	out = out.replace(/import\s*\{([^}]*)\}\s*from\s*"@anxionos\/contracts";/g, (_match, specifiers) => {
		...
		if (evaluation.length) parts.push(`import { ${evaluation.join(", ")} } from "@anxionos/contracts/evaluation";`);
		return parts.join("\n");
	});
