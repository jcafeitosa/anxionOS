...
export function isGateBindingVigente(binding: GateBinding, now: Date): boolean {
	if (binding.invalidatedAt && binding.invalidatedAt <= now) return false;
	return binding.disposition === "PASS";
}
export function shouldInvalidatePriorPass(
	existing: GateBinding,
	nextDigest: string | null,
): boolean {
	if (existing.disposition !== "PASS" || existing.invalidatedAt) return false;
	if (!nextDigest || !existing.artifactDigest) return false;
