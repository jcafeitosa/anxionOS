...
export interface OrchestrationCommandOutcome<TResponse = Record<string, unknown>> {
    response: TResponse;
    events: DomainEventEnvelope[];
}
export interface OrchestrationUnitOfWork {
    runInTransaction<T>(work: (context: OrchestrationTransactionContext) => Promise<T>): Promise<T>;
}
