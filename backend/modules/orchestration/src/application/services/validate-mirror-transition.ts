...
export interface ValidateMirrorTransitionDeps {
	...
	taskboardMirrorRepository: Pick<
...
const MIRROR_STATUS_TRANSITIONS: Record<
	TaskboardMirrorStatus,
	TaskboardMirrorStatus[]
> = {
...
export function requiresG7ForMirrorStatus(
	status: TaskboardMirrorStatus,
): boolean {
	return status === "done";
}
export function canTransitionMirrorStatus(
	from: TaskboardMirrorStatus,
	to: TaskboardMirrorStatus,
): boolean {
	if (from === to) {
		return true;
	}
...
export async function validateMirrorTransition(
	deps: ValidateMirrorTransitionDeps,
	input: ValidateMirrorTransitionInput,
): Promise<void> {
	const latest = await deps.taskboardMirrorRepository.findLatestByIssue(
		input.issueIdentifier,
	);
	if (latest && input.boardVersion < latest.boardVersion) {
		throwOrchestrationError(
			"ORC_MIRROR_REJECTED",
			`Stale board version ${input.boardVersion} < ${latest.boardVersion}`,
		);
	}
	if (
		latest &&
		...
		const from = latest.status as TaskboardMirrorStatus;
		if (!canTransitionMirrorStatus(from, input.status)) {
			throwOrchestrationError(
				"ORC_MIRROR_REJECTED",
				`Invalid mirror transition ${from} -> ${input.status}`,
			);
		}
	}
	if (requiresG7ForMirrorStatus(input.status)) {
		const g7Pass = await deps.gateBindingRepository.findVigentePassByIssue(
			input.issueIdentifier,
			"G7",
		);
		if (!g7Pass) {
			throwOrchestrationError(
				"ORC_MIRROR_REJECTED",
				"Board done requires G7 PASS binding",
			);
		}
	}
}
