...
export interface AssertCheckoutAuthorizedInput {
	organizationId: string;
	issueIdentifier: string;
	agentId: string;
}

export async function assertCheckoutAuthorized(
	context: OrchestrationTransactionContext,
	deps: CheckoutAuthorizationDeps,
	input: AssertCheckoutAuthorizedInput,
): Promise<void> {
	...
	try {
		...
		if (evaluation.decision !== "ALLOW") {
			throwOrchestrationError(
				"ORC_CHECKOUT_DENIED",
				`Traversal T01 denied checkout for ${input.issueIdentifier}`,
			);
		}
	} catch (error) {
		...
		throwOrchestrationError(
			...
			"Traversal T01 unavailable for checkout",
			{ cause: error },
		);
	}
	const g0Pass = await context.gateBindingRepository.findVigentePass(
	...
	const mirror = await context.taskboardMirrorRepository.findLatestByIssue(
		input.issueIdentifier,
	);
	...
	throwOrchestrationError(
		"ORC_CHECKOUT_DENIED",
		`Checkout denied for ${input.issueIdentifier}: missing G0 PASS or board in_progress`,
	);
}
