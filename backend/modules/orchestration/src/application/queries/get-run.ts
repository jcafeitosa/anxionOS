...
export async function getRun(deps, input) {
    const run = await deps.runRepository.findById(input.organizationId, input.runId);
    if (!run) {
        throwOrchestrationError("ORC_RUN_NOT_FOUND", `Run ${input.runId} not found`);
    }
    return toRunDto(run);
}
