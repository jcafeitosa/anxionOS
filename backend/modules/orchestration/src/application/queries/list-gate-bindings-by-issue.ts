...
export async function listGateBindingsByIssue(deps, input) {
    const bindings = await deps.gateBindingRepository.listByIssue(input.organizationId, input.issueIdentifier);
    return {
        bindings: bindings
