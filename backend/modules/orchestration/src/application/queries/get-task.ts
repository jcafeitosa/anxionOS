...
export async function getTask(deps, input) {
    const task = await deps.taskRepository.findById(input.organizationId, input.taskId);
    if (!task) {
        throwOrchestrationError("ORC_TASK_NOT_FOUND", `Task ${input.taskId} not found`);
    }
