...
export class CommandJournalHashMismatchError extends Error {
    constructor(message = "Command journal request hash mismatch", options) {
        super(message, options);
        this.name = "CommandJournalHashMismatchError";
    }
}
function deterministicCommandUuid(seed) {
...
export async function loadIdempotentRecordGateDispositionSnapshot(commandJournal, commandId, requestHash) {
    ...
    if (typeof snapshot.bindingId !== "string" ||
...
export async function loadIdempotentCommandSnapshot(commandJournal, commandId, requestHash) {
    const existing = await commandJournal.findByCommandId(commandId);
    if (!existing?.responseSnapshot) {
        return null;
    }
    assertCommandJournalReplay(existing.responseSnapshot, requestHash);
    const snapshot = existing.responseSnapshot;
    if (typeof snapshot.aggregateId !== "string" ||
        typeof snapshot.revision !== "number") {
        return null;
    }
    return {
...
export async function loadIdempotentCheckoutResult(commandJournal, commandId) {
    const existing = await commandJournal.findByCommandId(commandId);
    if (!existing) {
        return null;
    }
