import { type CheckoutTaskCommand, type CheckoutTaskResult } from "@anxionos/contracts/orchestration";
...
import { checkoutTaskCommandSchema, checkoutTaskResultSchema } from "@anxionos/contracts/orchestration";
import { canTransitionCheckoutStatus } from "../../domain/entities/task";
import { DEFAULT_LEASE_TTL_MS, MAX_LEASE_TTL_MS, isLeaseActive, } from "../../domain/entities/task-lease";
...
import { buildCheckoutCommandId, loadIdempotentCheckoutResult, toCheckoutResultSnapshot, } from "../command-support";
import { toRunDto, toTaskDto } from "../dto-mappers";
import { parseCheckoutResultSnapshot, throwOrchestrationError } from "../errors";
...
export async function checkoutTask(deps, input) {
    const command = checkoutTaskCommandSchema.parse(input);
    const commandId = buildCheckoutCommandId(command.agentId, command.taskId);
    const replay = await loadIdempotentCheckoutResult(deps.commandJournal, commandId);
    if (replay) {
        return replay;
    }
    return deps.unitOfWork.runInTransaction(async (context) => {
        ...
        const taskWithLease = await context.taskRepository.findByIdForUpdate(command.organizationId, command.taskId);
        if (!taskWithLease) {
        ...
        if (activeLease && isLeaseActive(activeLease, now)) {
            if (activeLease.agentId !== command.agentId) {
                throwOrchestrationError("ORC_LEASE_CONFLICT", `Task ${command.taskId} is leased by another agent`);
            }
            const run = await context.runRepository.findActiveByTaskAndAgent(command.organizationId, command.taskId, command.agentId);
            if (!run) {
                throwOrchestrationError("ORC_RUN_NOT_FOUND", `Active run not found for task ${command.taskId}`);
            }
        ...
        if (taskWithLease.checkoutStatus !== "UNCLAIMED" &&
            !canTransitionCheckoutStatus(taskWithLease.checkoutStatus, "LEASED")) {
            throwOrchestrationError("ORC_CHECKOUT_DENIED", `Task ${command.taskId} checkout status is ${taskWithLease.checkoutStatus}`);
        }
        const ttlMs = Math.min(command.leaseTtlMs ?? DEFAULT_LEASE_TTL_MS, MAX_LEASE_TTL_MS);
        const leasedAt = now;
        const expiresAt = deps.leaseClock.expiresIn(ttlMs);
        const leaseId = randomUUID();
        const leaseToken = randomUUID();
        const runId = randomUUID();
        const coalesceKey = buildCheckoutCommandId(command.agentId, command.taskId);
