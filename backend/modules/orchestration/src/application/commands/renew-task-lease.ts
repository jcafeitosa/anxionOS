...
import { DEFAULT_LEASE_TTL_MS, MAX_LEASE_TTL_MS, isLeaseActive, leaseTokensMatch, } from "../../domain/entities/task-lease";
...
export async function renewTaskLease(deps, input) {
    ...
    return deps.unitOfWork.runInTransaction(async (context) => {
        ...
        const maxExpiresAt = new Date(lease.leasedAt.getTime() + MAX_LEASE_TTL_MS);
        const proposedExpiresAt = deps.leaseClock.expiresIn(DEFAULT_LEASE_TTL_MS);
        const expiresAt = proposedExpiresAt > maxExpiresAt ? maxExpiresAt : proposedExpiresAt;
        if (expiresAt <= now) {
            throwOrchestrationError("ORC_LEASE_EXPIRED", `Lease renewal cap reached for task ${command.taskId}`);
        }
