...
import { HEARTBEAT_COALESCE_WINDOW_MS, buildHeartbeatCoalesceKey, } from "../../domain/entities/run-heartbeat";
...
export async function recordRunHeartbeat(deps, input) {
    const command = recordRunHeartbeatCommandSchema.parse(input);
    const coalesceKey = buildHeartbeatCoalesceKey(command.taskId, command.agentId);
    ...
    return deps.unitOfWork.runInTransaction(async (context) => {
        const run = await context.runRepository.findById(command.organizationId, command.runId);
        if (!run || run.taskId !== command.taskId || run.agentId !== command.agentId) {
            throwOrchestrationError("ORC_RUN_NOT_FOUND", `Run ${command.runId} not found for heartbeat`);
        }
        if (run.status !== "ACTIVE" && run.status !== "WAKING" && run.status !== "PAUSED") {
            throwOrchestrationError("ORC_CHECKOUT_DENIED", `Run ${command.runId} is not active for heartbeat`);
        }
        const lease = await context.taskLeaseRepository.findActiveByTaskId(command.taskId);
        if (!lease || lease.runId !== command.runId || lease.agentId !== command.agentId) {
        ...
        const existing = await context.runHeartbeatRepository.findPendingByCoalesceKey(coalesceKey);
        if (existing) {
            const coalescedWakeAt = existing.nextWakeAt.getTime() > nextWakeAt.getTime()
                ? existing.nextWakeAt
                : nextWakeAt;
        ...
        const pendingCount = await context.runHeartbeatRepository.countPendingByOrganization(command.organizationId);
        if (pendingCount >= HEARTBEAT_QUEUE_CAP_PER_ORG) {
            throwOrchestrationError("ORC_HEARTBEAT_BACKPRESSURE", `Heartbeat queue cap ${HEARTBEAT_QUEUE_CAP_PER_ORG} reached for organization`);
        }
