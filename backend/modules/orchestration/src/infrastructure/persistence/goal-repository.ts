...
export function createDrizzleGoalRepository(db: NodePgDatabase<Record<string, unknown>>): GoalRepository {
    return {
        ...
        async save(goal) {
            const existing = await db
                .select()
                .from(goals)
                .where(and(eq(goals.id, goal.id), eq(goals.organizationId, goal.organizationId)))
                .limit(1);
            if (existing[0]) {
                const rows = await db
                    .update(goals)
