...
function toTask(row: import("./schema").TaskRow) {
	return {
		id: row.id,
...
function toTaskLease(row: import("./schema").TaskLeaseRow) {
	return {
		id: row.id,
		taskId: row.taskId,
		runId: row.runId,
...
export function createDrizzleTaskRepository(
	...
	return {
		...
		async findByIdForUpdate(organizationId, taskId) {
			...
			return {
				...toTask(taskRow),
				lease: leaseRows[0] ? toTaskLease(leaseRows[0]) : null,
			};
		},
			...
			const rows = await db
				...
				.where(
					and(
						...
						eq(tasks.issueIdentifier, issueIdentifier),
					),
				)
				.limit(1);
			return rows[0] ? toTask(rows[0]) : null;
