...
export function createDrizzleGateBindingRepository(db: NodePgDatabase<Record<string, unknown>>): GateBindingRepository {
    return {
        async append(binding) {
            ...
            const row = rows[0];
            if (!row)
                throw new Error("Failed to append gate binding");
        ...
        async findVigentePass(organizationId, issueIdentifier, gateId) {
            const rows = await db
                .select()
                .from(gateBindings)
                .where(and(eq(gateBindings.organizationId, organizationId), eq(gateBindings.issueIdentifier, issueIdentifier), eq(gateBindings.gateId, gateId), eq(gateBindings.disposition, "PASS"), isNull(gateBindings.invalidatedAt)))
        ...
        async findVigentePassByIssue(issueIdentifier, gateId) {
            const rows = await db
                .select()
                .from(gateBindings)
                .where(and(eq(gateBindings.issueIdentifier, issueIdentifier), eq(gateBindings.gateId, gateId), eq(gateBindings.disposition, "PASS"), isNull(gateBindings.invalidatedAt)))
                .orderBy(desc(gateBindings.recordedAt))
                .limit(1);
            return rows[0] ? toGateBinding(rows[0]) : null;
        },
        async listByIssue(organizationId, issueIdentifier) {
            const rows = await db
                .select()
                .from(gateBindings)
                .where(and(eq(gateBindings.organizationId, organizationId), eq(gateBindings.issueIdentifier, issueIdentifier)));
            return rows.map(toGateBinding);
        },
        async invalidatePassBindings(organizationId, issueIdentifier, gateId, invalidatedAt) {
            const rows = await db
                .update(gateBindings)
                .set({ invalidatedAt })
                .where(and(eq(gateBindings.organizationId, organizationId), eq(gateBindings.issueIdentifier, issueIdentifier), eq(gateBindings.gateId, gateId), eq(gateBindings.disposition, "PASS"), isNull(gateBindings.invalidatedAt)))
