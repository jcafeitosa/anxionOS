...
export function createDrizzleRunHeartbeatRepository(db: NodePgDatabase<Record<string, unknown>>): RunHeartbeatRepository {
    return {
        ...
        async findPendingByCoalesceKey(coalesceKey) {
            const rows = await db
                .select()
                .from(runHeartbeats)
                .where(and(eq(runHeartbeats.coalesceKey, coalesceKey), eq(runHeartbeats.status, "pending")))
                .limit(1);
            return rows[0] ? toRunHeartbeat(rows[0]) : null;
        },
        async countPendingByOrganization(organizationId) {
            const rows = await db
                ...
                .where(and(eq(runs.organizationId, organizationId), eq(runHeartbeats.status, "pending")));
        ...
        async findDuePending(limit, now) {
            const rows = await db
                .select()
                .from(runHeartbeats)
                .where(and(eq(runHeartbeats.status, "pending"), lte(runHeartbeats.nextWakeAt, now)))
        ...
        async save(heartbeat) {
            ...
            if (existing[0]) {
                ...
                const row = rows[0];
                if (!row)
                    throw new Error("Failed to update run heartbeat");
            ...
            try {
                ...
                const row = rows[0];
                if (!row)
                    throw new Error("Failed to create run heartbeat");
                return toRunHeartbeat(row);
            }
            catch (error) {
                if (error instanceof Error &&
                    error.message.includes("orchestration_run_heartbeats_coalesce_pending_uidx")) {
                    const pending = await this.findPendingByCoalesceKey(heartbeat.coalesceKey);
                    if (pending)
                        return pending;
                }
                throw error;
