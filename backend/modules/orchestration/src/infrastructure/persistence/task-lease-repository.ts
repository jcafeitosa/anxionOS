...
export function createDrizzleTaskLeaseRepository(db: NodePgDatabase<Record<string, unknown>>): TaskLeaseRepository {
    return {
        ...
        async findByTaskIdAndToken(taskId, leaseToken) {
            const rows = await db
                .select()
                .from(taskLeases)
                .where(and(eq(taskLeases.taskId, taskId), eq(taskLeases.leaseToken, leaseToken), isNull(taskLeases.releasedAt)))
                .limit(1);
            return rows[0] ? toTaskLease(rows[0]) : null;
        },
        async findExpiredActive(before, limit) {
            const rows = await db
                ...
                .where(and(isNull(taskLeases.releasedAt), lt(taskLeases.expiresAt, before)))
        ...
        async save(lease) {
            ...
            if (existing[0]) {
                ...
                const row = rows[0];
                if (!row)
                    throw new Error("Failed to update task lease");
            ...
            const row = rows[0];
            if (!row)
                throw new Error("Failed to create task lease");
            return toTaskLease(row);
        },
    };
}
