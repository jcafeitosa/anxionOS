...
function mergeImplFile(dtsPath, jsPath) {
	...
	if (isReexportOnly(js)) {
		return `${typeOnlySection(rawDts)}\n\n${js.trim()}\n`;
	}
...
function fixImports(content) {
	let out = content.replace(
		...
		(_m, specifiers) =>
			`import type {${specifiers}} from "@anxionos/contracts/orchestration";`,
	);
	out = out.replace(
		...
		(_match, specifiers) => {
			...
			for (const name of names) {
				...
				if (bare === "AppError") errors.push(name);
				else if (
			...
			if (errors.length)
				parts.push(
					`import { ${errors.join(", ")} } from "@anxionos/contracts/errors";`,
				);
			if (events.length)
				parts.push(
			...
			if (orch.length)
				parts.push(
					`import { ${orch.join(", ")} } from "@anxionos/contracts/orchestration";`,
				);
			return parts.join("\n");
		},
	);
...
function reconstruct() {
	...
	walk(distRoot, (file) => {
		...
		if (!hasImpl || !fs.existsSync(jsPath)) {
			...
			content = mergeImplFile(file, jsPath);
		}
