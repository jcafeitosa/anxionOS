...
function fixImports(content) {
	let out = content.replace(/import type \{([^}]*)\} from "@anxionos\/contracts";/g, (_m, specifiers) => {
		...
		if (decisions.length) parts.push(`import type { ${decisions.join(", ")} } from "@anxionos/contracts/decisions";`);
		return parts.join("\n");
	});
	out = out.replace(/import\s*\{([^}]*)\}\s*from\s*"@anxionos\/contracts";/g, (_match, specifiers) => {
		...
		if (decisions.length) parts.push(`import { ${decisions.join(", ")} } from "@anxionos/contracts/decisions";`);
		return parts.join("\n");
	});
