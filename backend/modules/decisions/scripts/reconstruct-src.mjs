...
function fixImports(content) {
	let out = content.replace(
		...
		(_m, specifiers) => {
			const { events, decisions } = splitContractImports(specifiers);
			const parts = [];
			if (events.length)
				parts.push(
			...
			if (decisions.length)
				parts.push(
					`import type { ${decisions.join(", ")} } from "@anxionos/contracts/decisions";`,
				);
			return parts.join("\n");
		},
	);
	out = out.replace(
		...
		(_match, specifiers) => {
			...
			if (decisions.length)
				parts.push(
					`import { ${decisions.join(", ")} } from "@anxionos/contracts/decisions";`,
				);
			return parts.join("\n");
		},
	);
