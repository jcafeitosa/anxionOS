...
export function createPgDecisionRepository(client: PoolClient): DecisionRepository {
    return {
        async findById(id) {
            const result = await client.query(`SELECT * FROM decisions_records WHERE id = $1`, [id]);
            const row = result.rows[0];
            return row ? mapDecision(row) : null;
        },
        async save(record: DecisionRecord) {
            await client.query(`INSERT INTO decisions_records (
        ...
        async updateStatus(id, status, revision) {
            const result = await client.query(`UPDATE decisions_records SET status = $2, revision = $3 WHERE id = $1 RETURNING *`, [id, status, revision]);
            const row = result.rows[0];
            if (!row) {
                throw new DecisionsCommandError("DC_DECISION_NOT_FOUND", "decision not found for update");
            }
...
export function createPgProposalRepository(client: PoolClient): ProposalRepository {
    return {
        async findOpenByDecisionId(decisionId) {
            const result = await client.query(`SELECT * FROM decisions_proposals WHERE decision_id = $1 AND status = 'OPEN' LIMIT 1`, [decisionId]);
            const row = result.rows[0];
            return row ? mapProposal(row) : null;
        },
        async save(record: ProposalRecord) {
            await client.query(`INSERT INTO decisions_proposals (
...
export function createPgTradeIntentRepository(client: PoolClient): TradeIntentRepository {
    return {
        async findByDecisionId(decisionId) {
            const result = await client.query(`SELECT * FROM decisions_trade_intents WHERE decision_id = $1`, [decisionId]);
            const row = result.rows[0];
            return row ? mapTradeIntent(row) : null;
        },
        async save(record: TradeIntentRecord) {
            try {
                await client.query(`INSERT INTO decisions_trade_intents (
                ...
                if (isPgUniqueViolation(error)) {
                    throw new DecisionsCommandError("DC_INTENT_IMMUTABLE", "trade intent already submitted");
                }
                throw error;
            }
        },
