...
export interface DecisionRepository {
    ...
    updateStatus(id: string, status: string, revision: number): Promise<DecisionRecord>;
}
...
export interface DecisionsUnitOfWork {
    runInTransaction<T>(work: (ctx: DecisionsTransactionContext) => Promise<T>): Promise<T>;
}
