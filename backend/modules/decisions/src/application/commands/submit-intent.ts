import { type SubmitIntentCommand, type DecisionsCommandResult } from "@anxionos/contracts/decisions";
import type { CommandJournalRepository } from "../../domain/ports/command-journal";
import type { DecisionsUnitOfWork } from "../../domain/ports/decisions-unit-of-work";
import { randomUUID } from "node:crypto";
import { assertDecisionsExecutionModeSupported, decisionsCommandResultSchema, submitIntentCommandSchema } from "@anxionos/contracts/decisions";
...
export async function submitIntent(deps: SubmitIntentDeps, input: SubmitIntentCommand): Promise<DecisionsCommandResult> {
    ...
    return deps.unitOfWork.runInTransaction(async (ctx) => {
        ...
        const existingIntent = await ctx.tradeIntents.findByDecisionId(command.decisionId);
        if (existingIntent) {
        ...
        const updated = await ctx.decisions.updateStatus(decision.id, "SUBMITTED", decision.revision + 1);
