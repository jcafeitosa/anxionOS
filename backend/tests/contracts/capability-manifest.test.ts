...
describe("capability manifest catalog v1", () => {
    test("catalog parses and contains P02 foundation entries", () => {
        const parsed = parseCapabilityManifestCatalog(CAPABILITY_MANIFEST_V1_CATALOG);
    ...
    test("every entry exposes owner, grants, modes, effect, idempotency, approval and surfaces", () => {
        for (const entry of CAPABILITY_MANIFEST_V1_ENTRIES) {
            expect(entry.capabilityId).toMatch(/^[a-z][a-z0-9-]*(\.[a-z][a-zA-Z0-9-]*){1,2}$/);
...
describe("capability manifest registry", () => {
    ...
    test("rejects unknown capability", () => {
        expect(() => getCapabilityManifestEntry("unknown.capability.missing")).toThrow(
            CapabilityManifestError,
        );
    });

    test("deferred input schema fails explicitly", () => {
        expect(() => validateCapabilityInput("identity.session.revoke", { commandId: "x" })).toThrow(
            CapabilityManifestError,
        );
        try {
    ...
    test("UNKNOWN outcome is not treated as success", () => {
        expect(() => assertCapabilityOutcomeKnown("UNKNOWN", { capabilityId: "authorization.can" })).toThrow(
            CapabilityManifestError,
        );
        expect(() => assertCapabilityOutcomeKnown("RECONCILING")).toThrow(CapabilityManifestError);
...
describe("capability manifest client", () => {
    test("typed client enforces channel and mode", () => {
        ...
        const entry = client.assertInvocationAllowed("organizations.agency.create", {
            channel: "sdk",
...
describe("capability manifest pagination", () => {
    test("page request/response schemas validate boundaries", () => {
        const request = pageRequestSchema.parse({ pageSize: 25, pageToken: "cursor-1" });
