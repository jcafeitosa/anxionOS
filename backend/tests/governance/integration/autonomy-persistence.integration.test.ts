...
describe("governance autonomy persistence (slice 4 — migration 0005 + UoW)", () => {
	test.skipIf(Boolean(pgIntegrationSkipReason))(
		"ensureGovernanceSchema creates governance_autonomy_assignments table",
		async () => {
	...
	test.skipIf(Boolean(pgIntegrationSkipReason))(
		"assignAutonomyLevel persists assignment, command_journal, domain_journal and outbox",
		async () => {
			await withGovernancePgHarness(async ({ pool }) => {
         ...
         WHERE command_id = $1`,
				...
				expect(commandJournalRows.rows[0]?.aggregate_id).toBe(
					result.aggregateId,
				);
	...
	test.skipIf(Boolean(pgIntegrationSkipReason))(
		"transitionAutonomyLevel demote supersedes prior row and inserts new active assignment",
		async () => {
			await withGovernancePgHarness(async ({ pool }) => {
				const scopeId = randomUUID();
				const subjectAgentId = randomUUID();
