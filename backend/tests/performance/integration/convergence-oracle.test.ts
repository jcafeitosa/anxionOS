...
describe("ANX-154 convergence oracle (institutional fixture)", () => {
	test("end-to-end: derived metrics, PG persistence, and HTTP read queries converge", async () => {
		...
		await withPerformancePgHarness(async ({ pool, unitOfWork, commandJournal }) => {
			...
			expect(
				listedPositions.positionExposureSnapshots[0]?.positionExposureSnapshotId,
	...
	test("G5-PERF-02: divergent BUY ledger vs SHORT position is detected", async () => {
		...
		await withPerformancePgHarness(async ({ pool, unitOfWork, commandJournal }) => {
			...
			expect(divergence.some((reason) => reason.includes("signed_quantity"))).toBe(
				true,
			);
			expectDecimalEqual(
				positionMetrics.get(OFFICIAL_POSITION_EXPOSURE_METRICS.SIGNED_QUANTITY),
				"-1.5",
			);
			expectDecimalEqual(
