...
describe("SaaS plan catalog", () => {
	test("contains all four plans with entitlements", () => {
		...
		for (const planId of PLAN_IDS) {
			...
			expect(catalog[planId].entitlements.apiRequestsPerMinute).toBeGreaterThan(
				0,
			);
		}
	});
	...
	test("returns entitlements", () => {
		expect(getEntitlementsForPlan("freemium")).toEqual({
			maxAgents: 1,
			maxConnections: 1,
			maxStrategies: 1,
			maxAgencies: 1,
	...
	test("checks under, at, and over limit", () => {
		expect(checkEntitlement("beginner", "maxAgents", 2)).toEqual({
			allowed: true,
			limit: 3,
			current: 2,
			remaining: 1,
		});
		expect(checkEntitlement("beginner", "maxAgents", 3)).toEqual({
			allowed: true,
			limit: 3,
			current: 3,
			remaining: 0,
		});
		expect(checkEntitlement("beginner", "maxAgents", 4)).toEqual({
			allowed: false,
			limit: 3,
			current: 4,
			remaining: 0,
	...
	test("rejects invalid usage", () => {
		...
		expect(() => checkEntitlement("pro", "maxAgents", Number.NaN)).toThrow(
			RangeError,
		);
	});
	...
	test("has correct feature flags", () => {
		const catalog = createPlanCatalog();
		expect(catalog.freemium.features).toMatchObject({
			crypto: true,
			stocks: false,
		});
		expect(catalog.beginner.features).toMatchObject({
			crypto: true,
			stocks: false,
		});
		expect(catalog.trader.features).toMatchObject({
			crypto: true,
			stocks: true,
		});
		expect(catalog.pro.features).toMatchObject({
			crypto: true,
			stocks: true,
