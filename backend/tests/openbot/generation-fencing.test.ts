...
describe("generation fencing (ANX-144 S6 / R144-08)", () => {
	test("MEET oracle: acquire A → abort A → acquire B → release A → abort B → B still cancellable", async () => {
		...
		const acquiredA = await acquireBotRunGeneration({ botRunGeneration }, {
		...
		const abortedA = await abortBotRunGeneration({ botRunGeneration }, {
			commandId: randomUUID(),
		...
		expect(botRunGeneration.isGenerationAborted(generationA.generationId)).toBe(true);

		const acquiredB = await acquireBotRunGeneration({ botRunGeneration }, {
			commandId: randomUUID(),
			organizationId,
			agentId,
			runId,
		...
		expect(generationB.generationSequence).toBeGreaterThan(generationA.generationSequence);
		expect(botRunGeneration.isGenerationAborted(generationB.generationId)).toBe(false);

		const releasedA = await releaseBotRunGeneration({ botRunGeneration }, {
			commandId: randomUUID(),
			generationId: generationA.generationId,
			organizationId,
		});
		...
		const abortedB = await abortBotRunGeneration({ botRunGeneration }, {
			commandId: randomUUID(),
		...
		expect(botRunGeneration.isGenerationAborted(generationB.generationId)).toBe(true);
	});

	test("adversarial: abort A does not mark generation B as aborted", async () => {
		const { botRunGeneration } = createDeps();
		const genA = await acquireBotRunGeneration({ botRunGeneration }, {
			commandId: randomUUID(),
			organizationId,
			agentId,
			runId,
		...
		await abortBotRunGeneration({ botRunGeneration }, {
			commandId: randomUUID(),
		...
		const genB = await acquireBotRunGeneration({ botRunGeneration }, {
			commandId: randomUUID(),
			organizationId,
			agentId,
			runId,
		...
		expect(botRunGeneration.isGenerationAborted(genB.generation.generationId)).toBe(false);
	});
	...
	test("adversarial: stale abort token rejected without affecting active generation", async () => {
		...
		await expect(
			abortBotRunGeneration({ botRunGeneration }, {
				commandId: randomUUID(),
				generationId: genA.generation.generationId,
				organizationId,
	...
	test("adversarial: run revision mismatch rejected (orchestration fence)", async () => {
		...
		const acquired = await acquireBotRunGeneration({ botRunGeneration }, {
			commandId: randomUUID(),
			organizationId,
			agentId,
			runId,
		...
		runFence.setRunRevision({ organizationId, runId, runRevision: runRevision + 1 });
		await expect(
			abortBotRunGeneration({ botRunGeneration }, {
				commandId: randomUUID(),
...
describe("openbot S6 generation contracts (ANX-144 S6)", () => {
	...
	test("botRunGenerationRefSchema includes generationSequence and abortToken", async () => {
		const { botRunGeneration } = createDeps();
		const result = await acquireBotRunGeneration({ botRunGeneration }, {
			commandId: randomUUID(),
			organizationId,
			agentId,
			runId,
