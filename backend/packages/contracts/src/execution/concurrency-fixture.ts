...
import { EffectGateError, EFFECT_GATE_VIOLATION } from "./effect-gate";
...
export class ConcurrencyBoundaryError extends Error {
    code;
    constructor(code: ConcurrencyViolation | (typeof EFFECT_GATE_VIOLATION)[keyof typeof EFFECT_GATE_VIOLATION], message: string) {
        super(message);
...
export class ConcurrencyFixture {
    ...
    assertLeaseValid(lease: TaskLease, holderId: string, presentedLeaseToken: string, now: string) {
        if (lease.leaseToken !== presentedLeaseToken) {
            throw new ConcurrencyBoundaryError(CONCURRENCY_VIOLATION.LEASE_TOKEN_MISMATCH, "lease token mismatch");
        }
        if (lease.holderId !== holderId) {
            throw new ConcurrencyBoundaryError(CONCURRENCY_VIOLATION.LEASE_HOLDER_MISMATCH, "lease holder mismatch");
        }
        if (new Date(now) >= new Date(lease.expiresAt)) {
            throw new ConcurrencyBoundaryError(CONCURRENCY_VIOLATION.LEASE_EXPIRED, "lease expired");
        }
    }
    ...
    assertFirstDelivery(idempotencyKey: string) {
        if (this.idempotencyKeys.has(idempotencyKey)) {
            throw new ConcurrencyBoundaryError(CONCURRENCY_VIOLATION.DUPLICATE_DELIVERY, "duplicate idempotency key");
        }
    ...
    assertDispatchAllowed(permit: ExecutionPermit, idempotencyKey: string, now: string) {
        if (this.shutdown) {
            throw new ConcurrencyBoundaryError(CONCURRENCY_VIOLATION.SHUTDOWN, "system shutdown — no new dispatches");
        }
        ...
        if (this.revokedPermits.has(permit.permitId)) {
            throw new ConcurrencyBoundaryError(CONCURRENCY_VIOLATION.PERMIT_REVOKED, "permit revoked");
        }
        ...
        if (!this.tryConsumePermit(permit.permitId)) {
            throw new ConcurrencyBoundaryError(CONCURRENCY_VIOLATION.PERMIT_REUSED, "permit already consumed");
        }
    }
}
...
export type ConcurrencyViolation = (typeof CONCURRENCY_VIOLATION)[keyof typeof CONCURRENCY_VIOLATION];
