...
export class EffectGateError extends Error {
    code;
    constructor(code: (typeof EFFECT_GATE_VIOLATION)[keyof typeof EFFECT_GATE_VIOLATION], message: string) {
        super(message);
...
export function assertValidLifecycleTransition(from: OrderLifecycleState, to: OrderLifecycleState): void {
    const allowed = ALLOWED_TRANSITIONS[from];
    if (!allowed.includes(to)) {
        throw new EffectGateError(EFFECT_GATE_VIOLATION.INVALID_TRANSITION, `invalid transition ${from} -> ${to}`);
    }
}
...
export function assertEffectGateOpen(ctx: EffectGateContext): void {
    if (ctx.killSwitchActive) {
        throw new EffectGateError(EFFECT_GATE_VIOLATION.KILL_SWITCH, "kill switch active");
    }
    if (ctx.permitRevoked) {
        throw new EffectGateError(EFFECT_GATE_VIOLATION.PERMIT_REVOKED, "permit revoked");
    }
    if (ctx.policyStale) {
        throw new EffectGateError(EFFECT_GATE_VIOLATION.POLICY_STALE, "risk or governance policy stale");
    }
    if (ctx.permit.status !== "ACTIVE") {
        throw new EffectGateError(EFFECT_GATE_VIOLATION.PERMIT_NOT_ACTIVE, `permit status is ${ctx.permit.status}`);
    }
    if (new Date(ctx.now) >= new Date(ctx.permit.expiresAt)) {
        throw new EffectGateError(EFFECT_GATE_VIOLATION.PERMIT_EXPIRED, "permit expired");
    }
    if (isPermitStale(ctx.permit, ctx.currentAuthorityEpoch, ctx.currentRiskEpoch)) {
        throw new EffectGateError(EFFECT_GATE_VIOLATION.PERMIT_STALE, "permit epochs stale");
    }
    if ((ctx.effectClass === "EXTERNAL_EFFECT" ||
        ctx.effectClass === "IRREVERSIBLE") &&
        ctx.permit.singleUse !== true) {
        throw new EffectGateError(EFFECT_GATE_VIOLATION.EXTERNAL_WITHOUT_GATE, "external effect requires single-use permit");
    }
}
/** UNKNOWN and RECONCILING forbid blind retry without explicit reconcile decision. */
export function assertNoBlindRetry(currentState: OrderLifecycleState, hasReconcileDecision: boolean): void {
    if ((currentState === "UNKNOWN" || currentState === "RECONCILING") &&
        !hasReconcileDecision) {
        throw new EffectGateError(EFFECT_GATE_VIOLATION.BLIND_RETRY, "reconcile before retry when state is UNKNOWN/RECONCILING");
    }
}
...
export type ReconcileUnknownCommand = z.infer<typeof reconcileUnknownCommandSchema>;

export type EffectGateViolation = (typeof EFFECT_GATE_VIOLATION)[keyof typeof EFFECT_GATE_VIOLATION];
