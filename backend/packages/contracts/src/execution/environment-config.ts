...
function findAdapter(config: ExecutionEnvironmentConfig, adapterId: string) {
	...
	if (!adapter) {
		throw new ExecutionEnvironmentBoundaryError(
			...
			`adapter ${adapterId} not registered`,
		);
	}
	return adapter;
}
function findAccount(config: ExecutionEnvironmentConfig, accountId: string) {
	...
	if (!account) {
		throw new ExecutionEnvironmentBoundaryError(
			ENV_BOUNDARY_VIOLATION.PRODUCTION_ACCOUNT_DENIED,
...
function findSecretRef(
	...
	const secretRef = config.secretRefs.find(
		(s) => s.secretRefId === secretRefId,
	);
	if (!secretRef) {
		throw new ExecutionEnvironmentBoundaryError(
			ENV_BOUNDARY_VIOLATION.PRODUCTION_SECRET_DENIED,
			`secret ref ${secretRefId} not registered`,
		);
	}
	return secretRef;
}
/** Production accounts and REAL-mode accounts are unreachable from SIMULATED/PAPER. */
export function assertAccountAllowedForMode(
	mode: ExecutionMode,
	...
	if (account.isProduction || account.executionMode === "REAL") {
		throw new ExecutionEnvironmentBoundaryError(
			ENV_BOUNDARY_VIOLATION.PRODUCTION_ACCOUNT_DENIED,
			`account ${account.accountId} is production-only`,
		);
	}
	if (account.executionMode !== mode) {
		throw new ExecutionEnvironmentBoundaryError(
			ENV_BOUNDARY_VIOLATION.MODE_MISMATCH,
			`account ${account.accountId} is bound to ${account.executionMode}, not ${mode}`,
		);
	}
}
/** Secret refs with REAL_VENUE scope never resolve for SIMULATED/PAPER dispatch. */
export function assertSecretRefAllowedForMode(
	mode: ExecutionMode,
	secretRef: SecretRef,
): void {
	...
	if (!allowed.includes(secretRef.scope)) {
		throw new ExecutionEnvironmentBoundaryError(
			ENV_BOUNDARY_VIOLATION.PRODUCTION_SECRET_DENIED,
			`secret scope ${secretRef.scope} denied for mode ${mode}`,
		);
	}
}
/** Full dispatch bundle validation — negative proof entry point for boundary tests. */
export function assertDispatchBundleAllowed(
	mode: ExecutionMode,
 ...
 */
export function scanEnvironmentConfigForLeaks(
	config: ExecutionEnvironmentConfig,
): ConfigLeakFinding[] {
	const findings = [];
	for (const adapter of config.adapters) {
		if (
			adapter.secretScope === "REAL_VENUE" &&
...
export function parseExecutionEnvironmentConfig(
	input: unknown,
...
export type ExecutionEnvironmentConfig = z.infer<
