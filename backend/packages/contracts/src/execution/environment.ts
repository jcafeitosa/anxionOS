...
export function isAdapterAllowedForMode(
	mode: ExecutionMode,
...
export function assertNoSilentModeEscalation(
	intentMode: ExecutionMode,
	dispatchMode: ExecutionMode,
): void {
	if (intentMode !== dispatchMode) {
		throw new ExecutionEnvironmentBoundaryError(
			ENV_BOUNDARY_VIOLATION.SILENT_ESCALATION,
			`executionMode cannot change from ${intentMode} to ${dispatchMode}`,
		);
	}
}
/** Enforce adapter + secret scope boundaries before venue dispatch. */
export function assertAdapterAllowedForMode(
	mode: ExecutionMode,
	adapter: ExecutionAdapterDescriptor,
): void {
	if (!adapter.supportedModes.includes(mode)) {
		throw new ExecutionEnvironmentBoundaryError(
			ENV_BOUNDARY_VIOLATION.ADAPTER_NOT_ALLOWED,
			`adapter ${adapter.adapterId} does not support mode ${mode}`,
		);
	}
	if (mode === "REAL" && !adapter.homologatedForReal) {
		throw new ExecutionEnvironmentBoundaryError(
			ENV_BOUNDARY_VIOLATION.REAL_NOT_HOMOLOGATED,
			`adapter ${adapter.adapterId} is not homologated for REAL`,
		);
	}
	const allowedScopes = SECRET_SCOPE_BY_MODE[mode];
	if (!allowedScopes.includes(adapter.secretScope)) {
		throw new ExecutionEnvironmentBoundaryError(
			ENV_BOUNDARY_VIOLATION.SECRET_SCOPE_DENIED,
			`secret scope ${adapter.secretScope} denied for mode ${mode}`,
		);
	}
}
export function parseExecutionAdapterDescriptor(
	input: unknown,
...
export type ExecutionAdapterDescriptor = z.infer<
	typeof executionAdapterDescriptorSchema
>;

export type EnvBoundaryViolation =
	(typeof ENV_BOUNDARY_VIOLATION)[keyof typeof ENV_BOUNDARY_VIOLATION];
