...
const entryIndex = new Map<CapabilityId, CapabilityManifestEntry>(
    CAPABILITY_MANIFEST_V1_CATALOG.entries.map((entry) => [entry.capabilityId, entry]),
);

export function parseCapabilityManifestCatalog(input: unknown): CapabilityManifestCatalog {
    return capabilityManifestCatalogSchema.parse(input);
}

export function getCapabilityManifestEntry(capabilityId: CapabilityId): CapabilityManifestEntry {
...
export function validateCapabilityInput(capabilityId: CapabilityId, input: unknown): unknown {
...
export function parseCapabilityOutput(capabilityId: CapabilityId, output: unknown): unknown {
    const entry = getCapabilityManifestEntry(capabilityId);
...
export function parseCapabilityError(capabilityId: CapabilityId, errorBody: unknown): unknown {
    const entry = getCapabilityManifestEntry(capabilityId);
