...
export const adapterDispatchIdSchema = z.string().regex(/^agw_dsp_[0-9a-f-]{36}$/i);
/** First increment: SIMULATED only (ANX-117). */
export const adapterGatewayEnvironmentSchema = z.enum(["SIMULATED"]);
export const adapterGatewayExecutionModeSchema = z.enum(["SIMULATED", "PAPER", "REAL"]);
...
export function assertAdapterManifestSupportsCapabilities(manifest: AdapterManifest, requested: AdapterCapabilityKey[]): void {
    for (const capability of requested) {
        if (!manifest.capabilities[capability]) {
            throw new AdapterGatewayContractError(`AGW_CAPABILITY_MISSING:${capability}`);
        }
    }
}

export type AdapterDispatchId = z.infer<typeof adapterDispatchIdSchema>;
export type AdapterGatewayEnvironment = z.infer<typeof adapterGatewayEnvironmentSchema>;
export type AdapterGatewayExecutionMode = z.infer<typeof adapterGatewayExecutionModeSchema>;
