...
export function isAppError(error: unknown): error is AppError {
    ...
    return (ERROR_CODES.includes(candidate.code as ErrorCode) &&
...
export function toErrorResponse(error: unknown, options: ToErrorResponseOptions = {}): ErrorResponse {
    ...
    const exposeStack = options.exposeStack ?? process.env.EXPOSE_ERROR_DETAILS === "true";
    if (isAppError(error)) {
        const body = {
            code: error.code,
    ...
    const elysiaCode = isRecord(error) && "code" in error
        ? String(error.code)
        : "UNKNOWN";
    const mappedCode = mapElysiaCode(elysiaCode);
    const validationDetails = mappedCode === "VALIDATION_ERROR"
        ? extractValidationDetails(error)
        : undefined;
    const message = exposeDetails && error instanceof Error
        ? error.message
