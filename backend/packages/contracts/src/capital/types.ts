...
export const capitalAccountIdSchema = z
	.string()
	.regex(/^cap_acc_[0-9a-f-]{36}$/i);
export const capitalAllocationIdSchema = z
	.string()
	.regex(/^cap_alloc_[0-9a-f-]{36}$/i);
export const capitalReservationIdSchema = z
	.string()
	.regex(/^cap_res_[0-9a-f-]{36}$/i);
...
export const capitalReservationStatusSchema = z.enum([
	"HELD",
	"CONSUMED",
	"RELEASED",
	"EXPIRED",
]);
export const capitalReservationKindSchema = z.enum([
	"ORDER",
	"FEE_BUFFER",
	"MARGIN",
]);
