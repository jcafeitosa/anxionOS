...
import { billingSubscriptionIdSchema, billingPeriodSchema, decimalAmountSchema } from "./types";
/** Bridge schema for billing consumer input shaped as connections.usage.recorded.v1. */
export const connectionsUsageRecordedBridgeSchema = usageRecordedPayloadSchema.extend({
    organizationId: z.string().uuid(),
    subscriptionId: billingSubscriptionIdSchema,
    billingPeriod: billingPeriodSchema,
    unitPrice: decimalAmountSchema,
});
export function mapUsageRecordedToBillingInput(usage: ConnectionsUsageRecordedBridge, commandId: string): RecordUsageFromConnectionsInput {
...
export type ConnectionsUsageRecordedBridge = z.infer<typeof connectionsUsageRecordedBridgeSchema>;
