...
const PARTNER_SECRET_PATTERN =
	/(?:api[_-]?key|authorization|bearer|credential|password|private[_-]?key|secret|token)\s*(?:[:=]|\s+)\s*[^\s,]+|(?:postgres(?:ql)?|mysql|mongodb(?:\+srv)?|redis|amqp):\/\/|-----BEGIN [A-Z ]+ PRIVATE KEY-----/i;
